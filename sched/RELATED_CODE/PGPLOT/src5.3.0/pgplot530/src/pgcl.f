...
C 13-Mar-2003 - add options for label orientation [TJP]
C-----------------------------------------------------------------------
      INCLUDE  'pgplot.inc'
...
C            -- find slope of this segment (ANGLE)
     ...
     :           ANGLE = 57.3*ATAN2((YY-YP)*DINDY, (XX-XP)*DINDX)
               IF (PGCLAN.EQ.1) THEN
C                 -- downhill orientation     
                  ANGLE = ANGLE+180.0
               ELSE IF (PGCLAN.EQ.2) THEN
C                 -- uppage orientation
                  IF (ANGLE.GT.180.0) ANGLE = ANGLE - 360.0
                  IF (ANGLE.LT.-180.0) ANGLE = ANGLE + 360.0
                  IF (ABS(ANGLE) .GT. 90.0) ANGLE = ANGLE+180.0
               END IF
            END IF
