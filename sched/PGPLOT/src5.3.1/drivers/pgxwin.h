...
struct PgxWin {
  ...
  int fix_size;             /* If true, after the first call to pgpage */
                            /*  the pixmap and window sizes will be left */
                            /*  unchanged whenever a new page is started. */
...
int pgx_fix_size ARGS((PgxWin *pgx, int fix_size));
...
#define PGX_HLINE_CURSOR 5 /* Horizontal line cursor at y=beg.y */
#define PGX_VLINE_CURSOR 6 /* Vertical line cursor at x=beg.x */
