...
C     sources with catalog entries.  That is filled by SRCFLG.
C
...
C
      ...
      INCLUDE    'schpeak.inc'
C
      INTEGER    ISCN, KSRC, PSRC, IGRP
      LOGICAL    GOTSS, GOTSD, GOTSP, GOTPTG, INCLPTG
C ---------------------------------------------------------------------
            ...
            DO IGRP = 1, NPKGRP
               IF( NPKSRC(IGRP) .GT. 0 ) THEN
                  DO PSRC = 1, NPKSRC(IGRP)
                     GOTPTG = .FALSE.
                     DO KSRC = 1, NSRC
                        IF( PKSRC(PSRC,IGRP) .EQ. SRCNAME(KSRC) ) THEN
                           GOTPTG = .TRUE.
                        END IF
                     END DO
                     IF( .NOT. GOTPTG ) THEN
