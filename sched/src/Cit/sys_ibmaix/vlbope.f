C*VLBOPE -- open a VLB data file [IBM-AIX]
C+
...
C
      ...
      ELSE IF (TYPE.EQ.'MODEL' .OR. TYPE.EQ.'TEXT') THEN
          ...
          CC = 'LIST'
      ELSE
          MESG = 'VLBOPE: invalid argument TYPE='//TYPE
          CALL ERROR(MESG)
      END IF
C
...
C
      ...
      ELSE IF (STATUS.EQ.'NEW') THEN
          ISTAT = 2
      ELSE
          MESG = 'VLBOPE: invalid argument STATUS='//STATUS
          CALL ERROR(MESG)
      END IF
C
...
C
      ELSE
          IF (ISTAT.EQ.1) THEN
              MESG = '++ Cannot find '//TYPE//' file: '//
     1                NAME(1:LEN1(NAME))
              CALL PUTOUT(MESG)
          ELSE
              MESG = '++ Cannot create '//TYPE//' file: '//
     1                NAME(1:LEN1(NAME))
              CALL PUTOUT(MESG)
          END IF
          CALL GERROR(STRING)
          MESG = '++ '//STRING(1:LEN1(STRING))
          CALL PUTOUT(MESG)
          RESULT = STRING
          VLBOPE = 0
      END IF
