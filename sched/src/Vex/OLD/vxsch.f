...
C
      ...
      INTEGER     TRANSTAR, TRANEND, TRANLEN
      REAL        STASPD(MANT) 
...
C              default tranlen should be 30 seconds               
               ...
               TRANSTAR = INTSTOP - (TRANLEN + GRABTIME(2,ISCN))
               TRANEND = INTSTOP - GRABTIME(2,ISCN)
            END IF
...
C              tranlen should be whole scan
               TRANLEN = INTSTOP
               TRANSTAR = INTSTOP - TRANLEN 
               TRANEND = INTSTOP 
            END IF
            IF ( DATATRAN ) THEN
C
...
C
                  IF( TRANTYPE .EQ. 'disk2file' .AND. ISCN .LT. NSCANS) 
     1                                                          THEN
...
C
     ...
     2       ' new scan, but these appear over 30min apart!')
         CALL WLOG( 1,'       This can be improved by splitting'//
     2       ' up individual tape passes in multile scans. ')
      END IF
      IF( WARNFS ) 
