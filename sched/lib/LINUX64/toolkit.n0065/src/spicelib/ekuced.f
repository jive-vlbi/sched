C$Procedure     EKUCED ( EK, update d.p. column entry )
...
C$ Abstract
C
C     Update a double precision column entry in a specified EK record.
C
C$ Disclaimer
C
...
C         DOUBLE PRECISION or TIME, the error SPICE(WRONGDATATYPE) will
C         be signalled.
C
...
C         a double precision class known to this routine, the error
...
C$ Examples
C
C     1)  Replace the value in the third record of the column DCOL in
C         the fifth segment of an EK file designated by HANDLE.  Set
C         the new value to 999.D0.
C
C            CALL EKUCED ( HANDLE, 5, 3, 'DCOL', 1, 999.D0, .FALSE. )
C
C
C     2)  Same as (1), but this time add a null value.  The argument
C         999.D0 is ignored because the null flag is set to .TRUE.
C
C            CALL EKUCED ( HANDLE, 5, 3, 'DCOL', 1, 999.D0, .TRUE. )
C
C
C     3)  Replace the entry in the third record of the column DARRAY in
C         the fifth segment of an EK file designated by HANDLE.  Set
C         the new value using an array DBUFF of 10 d.p. values.
C
C            CALL EKUCED ( HANDLE, 5, 3, 'DARRAY', 10, DBUFF, .FALSE. )
C
C$ Restrictions
C
C     None.
...
C$ Index_Entries
C
C     replace d.p. entry in an EK column
C     replace time entry in an EK column
C
C-&
...
C
C     This column had better be of double precision or `time' type.
C
      DTYPE  =  COLDSC(TYPIDX)
      ...
      IF (  ( DTYPE .NE. DP ) .AND. ( DTYPE .NE. TIME )  ) THEN
         ...
         CALL SETMSG ( 'Column # is of type #; EKUCED only works '    //
     .                 'with d.p. or TIME columns.  RECNO = #; '      //
     .                 'SEGNO = #; EK = #.'                            )
         CALL ERRCH  ( '#',  COLUMN                                    )
         CALL ERRINT ( '#',  DTYPE                                     )
         CALL ERRINT ( '#',  RECNO                                     )
...
C
      ...
      IF ( CLASS .EQ. 2 ) THEN
C
C        Class 2 columns contain scalar d.p. data.
C
         CALL ZZEKUE02 ( HANDLE, SEGDSC, COLDSC, RECPTR, DVALS, ISNULL )
 
 
      ELSE IF ( CLASS .EQ. 5 ) THEN
C
C        Class 5 columns contain array-valued d.p. data.
C
         CALL ZZEKUE05 ( HANDLE,  SEGDSC, COLDSC,
