C$Procedure   ZZEKRSC ( EK, read scalar, character )
 
      SUBROUTINE ZZEKRSC (  HANDLE,  SEGDSC,  COLDSC,  RECPTR,
     .                      ELTIDX,  CVLEN,   CVAL,    ISNULL, FOUND  )
 
C$ Abstract
C
C     Read scalar data from a character column in a specified EK
C     record.
C
C$ Disclaimer
C
...
C     CVLEN      O   Length of stored string.
...
C$ Detailed_Output
C
C     CVLEN          is the length of the stored character string value.
C                    This is the actual number of characters stored;
C                    CVLEN may be less than the declared length of
C                    the column to which the value belongs, if the
C                    column is declared to contain fixed-length strings.
C                    CVLEN is valid only when FOUND is set to .TRUE.
C
C     CVAL           is the specified column entry.  CVAL is valid only
...
C
C        Class 6 columns contain character string array entries.
C
         CALL ZZEKRD06 ( HANDLE,  SEGDSC,  COLDSC,  RECPTR,
     .                   ELTIDX,  ELTIDX,  CVAL,    ISNULL,  FOUND )
