C$Procedure      LNKSIZ ( LNK, size )
 
      INTEGER FUNCTION LNKSIZ ( POOL )
 
C$ Abstract
C
C     Return the size of a doubly linked list pool.
C
C$ Disclaimer
C
...
C
C     The function returns the size of the pool.
C
C$ Detailed_Input
C
...
C$ Detailed_Output
C
C     The function returns the size (total number of nodes) of the pool.
C
C$ Parameters
C
...
C$ Particulars
C
C     This routine allows an application program to determine the size
C     of a doubly linked list pool at run-time, without having to rely
C     on knowledge of the internals of the doubly linked list pool
C     structure.
C
C$ Examples
C
C     1)  Let POOL be a doubly linked list pool.  The total number of
C         nodes in a doubly linked list pool is set when the pool is
C         initialized by LNKINI.  This number is returned by LNKSIZ:
C
C            C
C            C     This sequence of calls will assign the value 100
C            C     to the variable SIZE:
C            C
C                  CALL LNKINI ( 100, POOL )
C                  SIZE = LNKSIZ ( POOL )
C
C$ Restrictions
C
...
C
C     Grab the pool size from the control area.
C
