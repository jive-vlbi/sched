C$Procedure     EKUCEI ( EK, update integer column entry )
 
      SUBROUTINE EKUCEI (  HANDLE,  SEGNO,  RECNO,  COLUMN,
     .                     NVALS,   IVALS,  ISNULL          )
 
C$ Abstract
C
C     Update an integer column entry in a specified EK record.
C
C$ Disclaimer
C
...
C$ Examples
C
C     1)  Replace the value in the third record of the column ICOL in
C         the fifth segment of an EK file designated by HANDLE.  Set
C         the new value to 999.
C
C            CALL EKUCEI ( HANDLE, 5, 3, 'ICOL', 1, 999, .FALSE. )
C
C
C     2)  Same as (1), but this time add a null value.  The argument
C         999 is ignored because the null flag is set to .TRUE.
C
C            CALL EKUCEI ( HANDLE, 5, 3, 'ICOL', 1, 999, .TRUE. )
C
C
C     3)  Replace the entry in the third record of the column IARRAY in
C         the fifth segment of an EK file designated by HANDLE.  Set
C         the new value using an array IBUFF of 10 values.
C
C            CALL EKUCEI ( HANDLE, 5, 3, 'IARRAY', 10, IBUFF, .FALSE. )
C
C$ Restrictions
C
C     None.
...
C$ Index_Entries
C
C     replace integer entry in an EK column
C
C-&
...
C
      ...
      IF ( DTYPE .NE. INT ) THEN
         ...
         CALL SETMSG ( 'Column # is of type #; EKUCEI only works '    //
...
C
         CALL ZZEKUE01 ( HANDLE, SEGDSC, COLDSC, RECPTR, IVALS, ISNULL )
...
C
         CALL ZZEKUE04 ( HANDLE,  SEGDSC, COLDSC,
