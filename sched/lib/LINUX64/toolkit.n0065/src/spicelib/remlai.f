C$Procedure      REMLAI ( Remove elements from an integer array )
 
      SUBROUTINE REMLAI ( NE, LOC, ARRAY, NA )
 
C$ Abstract
C
C      Remove one or more elements from an integer array at the
...
C
C      CALL REMLAI ( 3,  1, ARRAY, -1 )
C      CALL REMLAI ( 3, -1, ARRAY,  7 )
C      CALL REMLAI ( 3,  6, ARRAY,  7 )
C
C$ Restrictions
C
C      None.
...
C$ Index_Entries
C
C     remove elements from an integer array
C
C-&
