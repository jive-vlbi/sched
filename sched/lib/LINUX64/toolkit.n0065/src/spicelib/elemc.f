C$Procedure            ELEMC ( Element of a character set )
 
      LOGICAL FUNCTION ELEMC ( ITEM, A )
 
C$ Abstract
C
C      Determine whether an item is an element of a character set.
C
C$ Disclaimer
C
...
C
      ELEMC = ( BSRCHC ( ITEM, CARDC ( A ), A(1) ) .NE. 0 )
