...
C                 corresponding to frame FRAME2 to states relative
C                 to the frame FRAME2.  More explicitly, if STATE
...
C
C     2) If the auxiliary information needed to compute a non-inertial
C        frame is not available an error will be diagnosed and signaled
...
C        corresponding to the cross product linear mapping is:
C
C            -               -
C           |   0  -A3    A2  |
C           |                 |
...
C$ Version
C
C-    SPICELIB Version 2.0.1, 16-JAN-2014 (NJB)
C
C        Corrected equation 1 in header comments. Corrected
C        numerous spelling errors in comments.
C        
...
C     TRANS(1...6,1...6,I) has the transformation from FRAME(I)
