C$Procedure      REORDL ( Reorder a logical array )
 
      SUBROUTINE REORDL ( IORDER, NDIM, ARRAY )
 
C$ Abstract
C
C      Re-order the elements of a logical array according to
...
C
C                  Note that the order imposed by REORDL is not the
...
C                  logicals in unspecified order.
C
C$ Detailed_Output
C
C      ARRAY       on output, is the same array, with the logicals
C                  re-ordered as specified by IORDER.
C
C$ Parameters
C
C     None.
...
C$ Particulars
C
C      REORDL uses a cyclical algorithm to re-order the elements of
...
C
C      The order vector used by REORDL is typically created for
...
C      masses, asteroid flag, and visual magnitudes for a group
...
C$ Version
C
C-     SPICELIB Version 1.0.0, 6-MAR-1996 (WLT) (IMU)
C
C-&
 
C$ Index_Entries
C
C     reorder a logical array
C
C-&
