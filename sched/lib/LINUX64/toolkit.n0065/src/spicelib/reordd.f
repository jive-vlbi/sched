C$Procedure      REORDD ( Reorder a double precision array )
 
      SUBROUTINE REORDD ( IORDER, NDIM, ARRAY )
 
C$ Abstract
C
C      Re-order the elements of a double precision array according to
...
C
C                  Note that the order imposed by REORDD is not the
...
C$ Particulars
C
C      REORDD uses a cyclical algorithm to re-order the elements of
...
C
C      The order vector used by REORDD is typically created for
