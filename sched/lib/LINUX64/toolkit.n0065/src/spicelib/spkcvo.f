C$Procedure SPKCVO ( SPK, constant velocity observer state )
 
      SUBROUTINE SPKCVO ( TARGET, ET,     OUTREF, REFLOC, ABCORR,
     .                    OBSSTA, OBSEPC, OBSCTR, OBSREF, STATE,  LT )

C$ Abstract
C
C     Return the state of a specified target relative to an "observer,"
C     where the observer has constant velocity in a specified reference
C     frame.  The observer's state is provided by the calling program
...
C     OBSSTA     I   Observer state relative to center of motion.
C     OBSEPC     I   Epoch of observer state.
C     OBSCTR     I   Center of motion of observer.
C     OBSREF     I   Frame of observer state.
...
C
C                 ET is independent of the observer epoch OBSEPC.
C
C
...
C
C
C     OBSSTA      is the geometric state of an observer moving at
C                 constant velocity relative to its center of motion
C                 OBSCTR, expressed in the reference frame OBSREF, at
C                 the epoch OBSEPC.
C
C                 OBSSTA is a six-dimensional vector representing
C                 position and velocity in cartesian coordinates: the
C                 first three components represent the position of an
C                 observer relative to its center of motion; the last
...
C
C    
C     OBSEPC      is the epoch, expressed as seconds past J2000 TDB, at
C                 which the observer state OBSSTA is applicable. For
C                 other epochs, the position of the observer relative
C                 to its center of motion is linearly extrapolated
C                 using the velocity component of OBSSTA.
C
C                 OBSEPC is independent of the epoch ET at which the
C                 state of the target relative to the observer is to be
C                 computed.
C
C     OBSCTR      is the name of the center of motion of OBSSTA. The
...
C                 the input state OBSSTA is expressed. The observer has
C                 constant velocity relative to its center of motion 
...
C         SPICE(UNKNOWNFRAME) is signaled.
C
...
C           states of observer center and target relative to the solar
...
C
C     Observers supported by this routine must have constant velocity
...
C     loaded SPK data.
C
C     For applications in which the observer has zero velocity
C     relative to its center of motion, the SPICELIB routine 
C
C        SPKCPO     { SPK, constant position observer }
C
C     can be used. SPKCPO has a simpler interface than that of SPKCVO.
C     
...
C        slightly. This is due to differences in the orientations
C        of the topocentric frames. There are two sources of the
C        differences:
C
...
C        KPL/MK
C
C           File name: spkcvo.tm
C
C           This is the meta-kernel file for the header code example for
C           the subroutine SPKCVO. These kernel files can be found on
C           the NAIF website.
C
...
C        C     Program: EX1
C        C
C        C     This program uses SPKCVO to compute solar azimuth
C        C     and elevation at a given surface point on the earth:
C        C     the DSN station DSS-14.
C        C
C
C              IMPLICIT NONE
C        C
...
C        C     observer state relative to its center.
C        C
C              TARGET = 'SUN'
C              OBSCTR = 'EARTH'
C              OBSREF = 'ITRF93'
...
C              OBSEPC    =  0.D0
C
C              OBSSTA(1) =  -2353.6213656676991D0
C              OBSSTA(2) =  -4641.3414911499403D0
C              OBSSTA(3) =   3677.0523293197439D0
C              OBSSTA(4) =     -0.00000000000057086D0
C              OBSSTA(5) =      0.00000000000020549D0
C              OBSSTA(6) =     -0.00000000000012171D0
C
C        C
...
C        C
C              CALL SPKCVO ( TARGET, ET,     OUTREF, REFLOC,
C             .              ABCORR, OBSSTA, OBSEPC, OBSCTR,
C             .              OBSREF, STATE0, LT0            )
C
C        C
...
C              WRITE (*,*) 'Observer-center state time: ', EPCSTR
...
C        C     using SPKCVO and then convert the state to
...
C              F  = ( RE - RP ) / RE
C
C              CALL RECGEO ( OBSSTA, RE, F, OBSLON, OBSLAT, OBSALT )
C
C        C
...
C        Observer-center state time: 2000 JAN 01 12:00:00.000000 TDB
...
C              62512272.82076501   58967494.42506485 -122059095.46751761
...
C
C        Distance between last two positions (km):            3.07056970
C
...
C        C     Program: EX2
C        C
C        C     This program demonstrates the use of SPKCVO.
...
C        C     can be used to compute the central meridian longitude.
...
C
C        Observer-target position (km):
C             -53720675.37940782  -51381249.05338467  -18838416.34716543
C        Observer-target velocity (km/s):
...
C        Light time (s):           255.79885530
C
C        Distance between above positions (km):             115.21404098
...
C
C        Observer-target position (km):
C             -71445232.12767358    2312773.74168720   27766441.52046534
C        Observer-target velocity (km/s):
C                   155.65895286       5061.78618477          5.09447029
...
C$ Index_Entries
C
C     state relative to constant_velocity_observer
C     state relative to constant_velocity surface_point
...
C     an ID code.
C
      CALL ZZBODS2C ( SVCTR1, SVOCTR, SVCCDE, SVFND1,
     .                OBSCTR, CTRCDE, FOUND    )
...
C
C     Convert the input name of the target to an ID code.
C
      CALL ZZBODS2C ( SVCTR2, SVTARG, SVTCDE, SVFND2,
     .                TARGET, TRGCDE, FOUND    )
...
C
C     Store inputs required to evaluate the observer's state relative
C     to its center of motion.
C
      CALL ZZCVSSTA ( OBSSTA, CTRCDE, OBSEPC, OBSREF )

C
C     Below, the routine that uses the inputs stored by ZZCVSSTA is
C     ZZCVXSTA. ZZCVXSTA computes the geometric state of the observer
C     relative to its center of motion in the frame OBSREF.
C     
...
C
      IF ( .NOT. USELT )  THEN
C
C        We evaluate the observer frame at ET, regardless of 
C        the setting of EVLTYP.
C
         CALL ZZSPKFZO ( TRGCDE, ET,       OUTREF,
     .                   ABCORR, ZZCVXSTA, STATE,  LT )
...
C        the output frame evaluation epoch; this is what ZZSPKFZO
C        does.
C
         CALL ZZSPKFZO ( TRGCDE, ET,       OUTREF,
     .                   ABCORR, ZZCVXSTA, STATE,  LT )
...
C
         CALL ZZSPKFZO ( TRGCDE, ET,       'J2000', 
     .                   ABCORR, ZZCVXSTA, J2STAT,  LT )
...
C        because in this case we have to account for the rate of change
C        of light time between target and observer. Start out by
C        determining the sign of the light time correction.
C
         IF ( XMIT ) THEN
C
...
C
         CALL ZZSPKFAO ( TRGCDE, ET,       'J2000', 
     .                   ABCORR, ZZCVXSTA, J2STAT,  LT, DLT )
...
C            
C           We're using light time corrections here.
C 
C           Adjust the transformation to account for the rate of
C           change of observer-target light time.
C
...
C
         CALL SETMSG ( 'Output frame evaluation locus '
     .   //            '# was not recognized. [Coding error].'   )
         CALL ERRCH  ( '#',  REFLOC                              )
