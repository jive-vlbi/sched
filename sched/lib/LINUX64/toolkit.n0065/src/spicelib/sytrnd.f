C$Procedure      SYTRND (Transpose two values associated with a symbol)
 
      SUBROUTINE SYTRND ( NAME, I, J, TABSYM, TABPTR, TABVAL )
...
C                transposed. If this index is not valid ( I < 1 or
C                I > the dimension of NAME) the symbol table is not
C                modified.
C
C     J          is the index of the second associated value to be
C                transposed. If this index is not valid ( J < 1 or
C                J > the dimension of NAME) the symbol table is not
C                modified.
C
C     TABSYM,
C     TABPTR,
...
C     The call,
C
C     CALL SYTRND ( 'BODY4_POLE_RA', 2, 3, TABSYM, TABPTR, TABVAL )
C
...
C     The next call,
C
C     CALL SYTRND ( 'MEAN_ANOM', 2, 4, TABSYM, TABPTR, TABVAL )
C
...
C
            IF ( I .NE. J ) THEN
               ...
               CALL SWAPD ( TABVAL(LOCVAL+I-1), TABVAL(LOCVAL+J-1) )
