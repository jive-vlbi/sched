C$Procedure      ZZEKBS01 ( EK, begin segment, type 1 )
 
      SUBROUTINE ZZEKBS01 (  HANDLE,  TABNAM,  NCOLS,
     .                       CNAMES,  CDSCRS,  SEGNO  )
 
C$ Abstract
C
C     Start a new type 1 segment in an E-kernel.
C
C$ Disclaimer
C
...
C     the addition of a new type 1 segment.  Type 1 segments are
C     `ordinary' in the sense that they support record insertion,
C     column entry update, and record deletion operations.
C
C     Type 1 segments may contains columns of class 1 through 6.
C
C     By way of contrast, type 2 segments have fixed record counts.
C
C$ Examples
C
C     See EKBSEG.
...
C
C     Initialize the record tree.
C
      CALL ZZEKTRIT ( HANDLE, REC )
...
C
C        If the column is indexed, create a new index for this column.
C        Currently, data type 1 indexes are in vogue.  Set the
C        descriptor to indicate the data type and to point to the index.
C
         IF ( CDSCRS(IXTIDX,I) .NE. IFALSE ) THEN
C
C           ZZEKCIX1 will update the descriptor to indicate the type and
C           location of the new index.
C
            CALL ZZEKCIX1 ( HANDLE, CDSCRS(1,I) )
