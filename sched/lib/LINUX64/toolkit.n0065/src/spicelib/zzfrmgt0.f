C$Procedure      ZZFRMGT0 (Frame get transformation)
 
      SUBROUTINE ZZFRMGT0 ( INFRM, ET, XFORM, OUTFRM, FOUND )
...
C        routine ZZDYNFR0 requires the input frame ID rather than the
C        dynamic frame class ID. ZZDYNFR0 also requires the center ID
C        we found via the FRINFO call.

         CALL ZZDYNFR0 ( INFRM, CENT, ET, XFORM, OUTFRM )

C
C        The FOUND flag was set by FRINFO earlier; we don't touch
C        it here.  If ZZDYNFR0 signaled an error, FOUND will be set
...
C
     ...
     .   //          'supported in version # of ZZFRMGT0. You need '
