C$Procedure            SIZEC ( Size of a character cell )
 
      INTEGER FUNCTION SIZEC ( CELL )
 
C$ Abstract
C
C     Return the size (maximum cardinality) of a character cell.
C
C$ Disclaimer
C
...
C         SPICE(INVALIDCARDINALITY) is signalled.  SIZEC returns
C         an unspecified value in this case.
C
C     2)  If the input array has invalid size, the error
C         SPICE(INVALIDSIZE) is signalled.  SIZEC returns
C         an unspecified value in this case.
C
C$ Files
C
