C$Procedure      SYENQI ( Enqueue a value onto a symbol )
 
      SUBROUTINE SYENQI ( NAME, VALUE, TABSYM, TABPTR, TABVAL )
 
C$ Abstract
C
C     Enqueue a value onto a particular symbol in an integer
...
C     The call,
C
C     CALL SYENQI ( 'books', 12, TABSYM, TABPTR, TABVAL )
C
...
C     The next call,
C
C     CALL SYENQI ( 'desks', 23, TABSYM, TABPTR, TABVAL )
C
...
C                     24
C
C     Notice that the symbol "desks" was created by the last call.
C
C$ Restrictions
C
C     None.
...
C
      ELSE IF (  NVAL .GE. SIZEI ( TABVAL ) ) THEN
         ...
         CALL SETMSG ( 'SYENQI: The addition of the value $ to the '  //
