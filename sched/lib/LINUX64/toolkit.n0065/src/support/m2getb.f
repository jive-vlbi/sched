C$Procedure M2GETB ( META/2 --- get a named integer)
 
      SUBROUTINE M2GETB ( NAME, STRING, FOUND, INT  )
      IMPLICIT NONE
 
C$ Abstract
C
C     Select the first substring associated with a matched, named META/2
C     template word and put it parse it as an integer.
C
C$ Disclaimer
C
...
C     INT        O   matching integer extracted and parsed from STRING.
C
C$ Detailed_Input
C
...
C                specified by NAME and STRING and NTH could be
C                retrieved. Otherwise it will be returned with a value
C                of .FALSE.
C
C     INT        is the ID-code represented by the word of STRING that
C                was the first matched with the NAMEd META/2 template
C                word.
C
C$ Parameters
C
...
C
C     2) If the portion of STRING extracted cannot be parsed as an
C        body, the error 'META/2(CORRUPTEDBODYSPEC)' will be
C        signalled.
C
C$ Files
C
...
C        M2GETB  --- get and parse a body name
...
C            CALL M2GETB ( 'body1',     STRING, FOUND,  BODY1  )
C            CALL M2GETB ( 'body2',     STRING, FOUND,  BODY2  )
C            CALL M2GETB ( 'observer',  STRING, FOUND,  OBS    )
...
C$ Index_Entry
C
C     Extract first integer matching a named template word
C
C-&
...
C
C     First see if we've got a recognized name.
C
      CALL M2BODN2C ( STRING(B:E), MYINT, FOUND )
 
      IF ( .NOT. FOUND ) THEN
C
C        Try an integer.
C
         CALL NPARSI ( STRING(B:E), MYINT, ERROR, PNTER )
 
         IF ( ERROR .NE. ' ' ) THEN
            CALL CHKIN  ( 'M2GETB' )
            CALL SETMSG ( 'The item requested, ''#'',  was not a '
     .      //            'recognized body and could not be parsed '
     .      //            'as an integer. ' )
            CALL ERRCH  ( '#', STRING(B:E) )
 
            CALL SIGERR ( 'META/2(CORRUPTEDBODYNAME)' )
