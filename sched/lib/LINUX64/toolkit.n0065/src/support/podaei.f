C$Procedure      PODAEI ( Pod, append elements, integer )
 
      SUBROUTINE PODAEI ( ELEMS, N, POD )
      IMPLICIT NONE
...
C
      ...
      IF (  SIZEI( POD ) .LT. ( END + N )  ) THEN
 
         CALL SETMSG ( 'Cannot fit # elements into # spaces.' )
         CALL ERRINT ( '#', N                                 )
         CALL ERRINT ( '#', SIZEI(POD) - END                  )
