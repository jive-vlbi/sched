C$Procedure M2SELD ( META/2 --- select a named double precision number )
 
      SUBROUTINE M2SELD ( NAME, STRING, NTH, FOUND, DP   )
      IMPLICIT NONE
...
C                of STRING that was the NTH match with the NAMEd META/2
C                template word.
C
C$ Parameters
C
...
C$ Index_Entry
C
C     Extract n'th number matching a named template word
C
C-&
