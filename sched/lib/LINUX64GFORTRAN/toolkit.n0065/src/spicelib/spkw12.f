C$Procedure      SPKW12 ( SPK, write segment, type 12 )
...
C$ Parameters
C
C     See the include file spk12.inc for declarations of the 
...
C                    of a type 12 segment, expressed as seconds past
C                    J2000 TDB.
C
...
C
C
C     MAXDEG         is the maximum degree of Hermite polynomials that
...
C         SPICE(BADDESCRTIMES) will be signaled.
C
C    10)  If the start time of the first record exceeds the descriptor
...
C$ Version
C
C-    SPICELIB Version 2.0.0, 18-JAN-2013 (NJB)
C
...
C
C        Corrected long error message for even degree case.
C
...
C
      IF (  EVEN( DEGREE )  )  THEN
         ...
         CALL SETMSG ( 'The interpolating polynomials have degree #; '//
     .                 'for SPK type 12, the degree must be odd.'      )
         CALL ERRINT ( '#', DEGREE                                     )
         CALL SIGERR ( 'SPICE(INVALIDDEGREE)'                          )
         CALL CHKOUT ( 'SPKW12'                                        )
         RETURN
