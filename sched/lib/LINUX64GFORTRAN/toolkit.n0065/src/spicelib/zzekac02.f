C$Procedure     ZZEKAC02 ( EK, add class 2 column to segment )
 
      SUBROUTINE ZZEKAC02 (  HANDLE,  SEGDSC,  COLDSC,  DVALS,
     .                       NLFLGS,  RCPTRS,  WKINDX         )
 
C$ Abstract
C
C     Add an entire class 2 column to an EK segment.
C
C$ Disclaimer
C
...
C
C     This column had better be class 2.
C
      IF ( CLASS .NE. 2 ) THEN
     ...
     .                 'column #.  Class should be 2.'                 )
         CALL ERRINT ( '#', CLASS                                      )
         CALL ERRCH  ( '#', COLUMN                                     )
         CALL SIGERR ( 'SPICE(NOCLASS)'                                )
         CALL CHKOUT ( 'ZZEKAC02'                                      )
         RETURN
