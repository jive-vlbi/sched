...
C      strings, find the index of the largest array element less
C      the given string.
C
C$ Disclaimer
C
...
C      LSTLTC     O   the index of the last element of ARRAY < STRING.
C
C$ Detailed_Input
C
C      STRING  Character string for which one desires to find
C              the last ARRAY element less than STRING.
C
C      N       Total number of elements in ARRAY.
C
C      ARRAY   Ordered array of character strings.
C              We will find the last element
C              of the sequence that is less than STRING.
C
C$ Detailed_Output
C
C      LSTLTC  Index of the last element of the ordered array:
C              {ARRAY(I) : 0 < I < N + 1} that is less than STRING.
C              (Note that LSTLTC = I for some I in the range 1 to
C              N  unless STRING is less than or equal to all of these
C              elements, in which case LSTLTC = 0.)
C
C              In the case that N is input with value less than or equal
C              to zero, LSTLTC is returned as zero.
C
C$ Parameters
C
C      None.
...
C      Given a real number STRING, there will be a last one of
C      these that is less than STRING.  This routine
C      finds the index LSTLTC such that ARRAY(LSTLTC) is that string.
C
C      If STRING is not greater than ARRAY(1), LSTLTC will be set to
C      zero.
C
...
C             is greater than or equal to STRING, simply add 1 to the
