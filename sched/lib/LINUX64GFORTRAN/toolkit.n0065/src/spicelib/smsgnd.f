C$Procedure            SMSGND  ( Same Sign Double Precision Numbers )
 
      LOGICAL FUNCTION SMSGND  ( X, Y )
...
C$ Detailed_Output
C
C      SMSGND is returned as .TRUE. if X and Y are both positive or both
C             negative.
C
C$ Parameters
C
...
C      Double Precision values having the same sign.
C
C      IF ( SMSGND ( F(X1), F(X2) ) ) THEN
C            .
C            .
C         do something
C            .
...
C$ Index_Entries
C
C     same sign d.p. numbers
C
C-&
 
 
      SMSGND = (      (( X .GT. 0 ) .AND. ( Y .GT. 0 ))
