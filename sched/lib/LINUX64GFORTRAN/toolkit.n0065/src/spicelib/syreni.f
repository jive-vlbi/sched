C$Procedure      SYRENI ( Rename an existing symbol )
 
      SUBROUTINE SYRENI ( OLD, NEW, TABSYM, TABPTR, TABVAL )
 
C$ Abstract
C
C     Rename an existing symbol in an integer symbol table.
C
C$ Disclaimer
C
...
C     The call,
C
C     CALL SYRENI ( 'pens', 'desks', TABSYM, TABPTR, TABVAL )
C
...
C     The next call,
C
C     CALL SYRENI ( 'erasers', 'desks', TABSYM, TABPTR, TABVAL )
C
C     modifies the contents fo the symbol table to be:
C
C        books   -->   5
C                     10
C        desks   -->   6
...
C                     18
C
C     Note that the symbol "desks" was deleted from the table,
C     and the symbol "erasers" was then renamed to "STRASSMAN". If the
...
C     The next call,
C
C     CALL SYRENI ( 'chairs', 'stools', TABSYM, TABPTR, TABVAL )
C
C     does not modify the contents of the symbol table. However, it
C     does signal the error SPICE(NOSUCHSYMBOL) because the symbol
C     "chairs" does not exist in the symbol table.
C
C$ Restrictions
C
C     None.
...
C
C         Changed the call to SYDELD to a call to SYDELI. The variable
...
C
      IF ( OLDLOC .EQ. 0 ) THEN
         ...
         CALL SETMSG ( 'SYRENI: The symbol # is not in the symbol ' //
     .                 'table.' )
         CALL ERRCH  ( '#', OLD )
...
C
         CALL SYDELI ( NEW, TABSYM, TABPTR, TABVAL )
...
C
         ...
         CALL SWAPAI ( OLDDIM, OLDVAL, 0, NEWVAL, TABVAL(1) )
