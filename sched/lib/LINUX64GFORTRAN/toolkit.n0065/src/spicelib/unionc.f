C$Procedure      UNIONC ( Union two character sets )
 
      SUBROUTINE UNIONC ( A, B, C )
 
C$ Abstract
C
C      Union two character sets to form a third set.
C
C$ Disclaimer
C
...
C
C        Made CHKOUT calls consistent with CHKIN.
C
...
C
         IF ( CCARD .LT. CSIZE ) THEN
            ...
            ELSE IF ( LGT ( A(APOINT), B(BPOINT) ) ) THEN
