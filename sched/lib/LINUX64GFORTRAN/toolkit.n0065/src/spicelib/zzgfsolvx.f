C$Procedure ZZGFSOLVX ( Private --- GF, event finding routine )
      ...
      SUBROUTINE ZZGFSOLVX ( UDFUNS, UDFUNB, UDSTEP,  UDREFN,  BAIL,
...
C     --------  ---  --------------------------------------------------
C     UDFUNS     I   The routine that computes the scalar quantity of
C                    interest.
C     UDFUNB     I   Name of the routine that compares the current state
...
C                           at ET.
C
C     UDFUNB     the routine that determines if UDFUNS
...
C
C                   BOOL     a logical value indicating whether
C                            or not UDFUNS satisfies the constraint
...
C$ Examples
C
C      See GFUDS and ZZGFRELX.
C
C$ Restrictions
C
C      It is important that the user understand how the routines
C      UDFUNB, UDSTEP and UDREFN are to be used and that the
