C$Procedure SPKS09 ( S/P Kernel, subset, type 9 )
...
C$ Abstract
C
C     Extract a subset of the data in an SPK segment of type 9
...
C     routine SPKSUB.
C
C     It transfers a subset of a type 09 SPK data segment to
C     a properly initialized segment of a second SPK file.
C
C     The exact structure of a segment of data type 09 is described
C     in the section on type 09 in the SPK Required Reading.
C
C$ Examples
C
...
C
C-    SPICELIB Version 2.0.0, 27-AUG-1994 (NJB)
C
...
C        segment.
C
C        Test for null subset simplified.
C
...
C
C     Get the number of records in the segment.  Get the polynomial
C     degree as well.
C
...
C
C     Store the polynomial degree and the number of records
