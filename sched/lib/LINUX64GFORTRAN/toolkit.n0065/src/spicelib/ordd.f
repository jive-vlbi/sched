C$Procedure      ORDD ( The ordinal position of an element in a set )
...
C$ Detailed_Input
C
C     ITEM      Is an DOUBLE PRECISION value to be located within a
C               set.
C
...
C
C     DOUBLE PRECISION      ARRAY (         N )
C     DOUBLE PRECISION      SET   ( LBCELL: N )
C     INTEGER               I
C
C     INTEGER               NVALID
C     INTEGER               POSITION
...
C     array ARRAY
C
C     CALL MOVED  ( ARRAY, N,      SET(1) )
C     CALL VALIDD ( N,     NVALID, SET    )
C
C     POSITION = ORDD ( ARRAY(I), SET )
C
...
C
      ORDD = BSRCHD ( ITEM, CARDD(SET), SET(1) )
