C$Procedure      LNKNFN ( LNK, number of free nodes )
 
      INTEGER FUNCTION LNKNFN ( POOL )
 
C$ Abstract
C
C     Return the number of free nodes in a doubly linked list pool.
C
C$ Disclaimer
C
...
C
C     The function returns the number of free nodes in the pool.
C
C$ Detailed_Input
C
...
C$ Particulars
C
C     This routine allows the caller to find the number of free nodes
C     available in a doubly linked list pool, without having to make
C     use of knowledge of the internal structure of the pool.
C
C     Routines that allocate nodes can use this routine to determine
C     how many nodes can be allocated safely---an attempt to allocate
C     a node when no free nodes are available causes a SPICELIB error
C     to be signalled.
C
C$ Examples
C
C     1)  Let POOL be a doubly linked list pool containing 5 nodes.
C         If POOL contains the list
C
C            4 <--> 5 <--> 1 <--> 2
C
C
C         and the node 3 is unallocated, then the function reference
C
C            NFREE  =  LNKNFN ( POOL )
C
C
C         will assign the value 1 to NFREE.
C
C
C$ Restrictions
C
...
C$ Index_Entries
C
C     return number of nodes in linked list pool
C
C-&
...
C
C     Grab the number of free nodes from the control area.
C
