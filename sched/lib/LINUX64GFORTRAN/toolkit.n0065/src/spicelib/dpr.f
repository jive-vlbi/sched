C$Procedure                     DPR ( Degrees per radian )
 
      DOUBLE PRECISION FUNCTION DPR ( )
 
C$ Abstract
C
C     Return the number of degrees per radian.
C
C$ Disclaimer
C
...
C$ Brief_I/O
C
C     The function returns the number of degrees per radian.
C
C$ Detailed_Input
C
C     None.
...
C$ Detailed_Output
C
C     The function returns the number of degrees per radian: 180/pi.
C     The value of pi is determined by the ACOS function. That is,
C
C           DPR = 180.D0 / ACOS ( -1.D0 )
C
C$ Parameters
C
C     None.
...
C$ Examples
C
C     The code fragment below illustrates the use of DPR.
C
C        C
C        C     Convert all angles to degrees for output.
C        C
C              CLOCK = CLOCK * DPR()
C              CONE  = CONE  * DPR()
C              TWIST = TWIST * DPR()
...
C              CALL VSCL   ( DPR(), ALBTGAM, ALBTGAM )
