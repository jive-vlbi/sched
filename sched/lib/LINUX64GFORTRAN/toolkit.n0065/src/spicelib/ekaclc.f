C$Procedure     EKACLC ( EK, add character column to segment )
 
      SUBROUTINE EKACLC (  HANDLE,  SEGNO,   COLUMN,  CVALS,
     .                     ENTSZS,  NLFLGS,  RCPTRS,  WKINDX  )
 
C$ Abstract
C
C     Add an entire character column to an EK segment.
C
C$ Disclaimer
C
...
C                    defined by CVALS and ENTSZS is added to the
...
C$ Index_Entries
C
C     write entire character column to EK segment
C
C-&
...
C
      ...
      IF ( DTYPE .NE. CHR ) THEN
         ...
         CALL SETMSG ( 'Column # is of type #; EKACLC only works '  //
...
C
      IF ( CLASS .EQ. 3 ) THEN
C
C        Class 3 columns contain character scalars.
C
         CALL ZZEKAC03 (  HANDLE,  SEGDSC,  COLDSC,  CVALS,
     .                             NLFLGS,  RCPTRS,  WKINDX )
 
      ELSE IF ( CLASS .EQ. 6 ) THEN
C
C        Class 6 columns contain character arrays.
C
         CALL ZZEKAC06 ( HANDLE, SEGDSC, COLDSC, CVALS, ENTSZS, NLFLGS )
 
 
      ELSE IF ( CLASS .EQ. 9 ) THEN
C
C        Class 9 columns contain fixed-count, fixed-length character
C        scalars.
C
         CALL ZZEKAC09 ( HANDLE, SEGDSC, COLDSC, CVALS, NLFLGS, WKINDX )
