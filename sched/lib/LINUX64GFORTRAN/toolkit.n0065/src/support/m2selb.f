C$Procedure M2SELB ( META/2 --- select a named integer )
 
      SUBROUTINE M2SELB ( NAME, STRING, NTH, FOUND, INT  )
      IMPLICIT NONE
 
C$ Abstract
C
C     Select the Nth substring associated with a matched, named META/2
C     template word and parse it as a body ID code.
C
C$ Disclaimer
C
...
C     NTH        I   the number(ordinal) of the word that matched.
C     FOUND      O   returned TRUE if the request could be fulfilled.
C     INT        O   ID-code extracted and parsed from STRING.
C
C$ Detailed_Input
C
...
C
C     NTH        is an ordinal number that specifies which of the
C                possible words in STRING that matched the named
C                template word is the one desired to extract
C                and parse.
C
C$ Detailed_Output
C
C     FOUND      will be returned .TRUE. if the requested information
C                specified by NAME, STRING and NTH could be retrieved.
C                Otherwise it will be returned with a value of .FALSE.
C
C     INT        is the ID-code associated with the word of STRING that
C                was the NTH match with the NAMEd META/2 template word.
C
C$ Parameters
C
C     None.
...
C        integer, the error 'META/2(CORRUPTEDINTEGER)' will be
C        signalled.
C
C$ Files
C
...
C$ Version
C
C-     META/2 Version 1.0.0, 18-AUG-1998 (WLT)
C-&
C
C$ Index_Entry
C
C     Extract n'th integer matching a named template word
C
C-&
...
C
      CALL M2VGET ( NAME, NTH, FOUND, B, E )
...
C
     ...
     .                 'it passed syntax validation in META/2. '      )
      ...
      END IF
C
C     First see if this is a recognized body...
C
...
C
C        This should be an integer double precision number.
C        Parse it.
C
     ...
     .      //            'as a body or body ID.'                  )
            CALL SIGERR ( 'META/2(CORRUPTEDBODY)'                  )
            CALL CHKOUT ( 'M2SELB' )
            RETURN
         END IF
