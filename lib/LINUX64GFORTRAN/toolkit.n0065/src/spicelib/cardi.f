C$Procedure            CARDI ( Cardinality of an integer cell )
 
      INTEGER FUNCTION CARDI ( CELL )
 
C$ Abstract
C
C     Return the cardinality (number of elements) of an integer cell.
C
C$ Disclaimer
C
...
C     cells. In the following example, SIZEI is used to determine
C     whether the integer cell ORIGINAL can be safely copied into
C     the integer cell SAVE before actually attempting the operation.
...
C         SPICE(INVALIDCARDINALITY) is signaled.  CARDI returns
C         an unspecified value in this case.
C
C     2)  If the input array has invalid size, the error
C         SPICE(INVALIDSIZE) is signaled.  CARDI returns
C         an unspecified value in this case.
C
C$ Files
C
...
C$ Index_Entries
C
C     cardinality of an integer cell
C
C-&
