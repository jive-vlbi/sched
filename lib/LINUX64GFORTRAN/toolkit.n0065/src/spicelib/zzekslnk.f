C$Procedure      ZZEKSLNK ( EK, set link count for data page )
 
      SUBROUTINE ZZEKSLNK ( HANDLE, TYPE, P, NLINKS )
 
C$ Abstract
C
C     Set the link count for a specified EK data page.
C
C$ Disclaimer
C
...
C     P          I   Page number.
C     NLINKS     I   Number of links to page.
C
C$ Detailed_Input
C
...
C                    routines.
C
C     NLINKS         is the new number of links to the specified data
C                    page.
C
C$ Detailed_Output
C
...
C$ Particulars
C
C     This routine updates the link count of the specified EK data page.
C
C     Link counts are used to indicate how many `users' of a page
C     there are.  When the link count of a page drops to zero, that
C     page is eligible to be freed.
C
C$ Examples
C
C     See EKDELR.
...
C
         CALL ZZEKSEI ( HANDLE, BASE+CLCIDX, NLINKS )
...
C
         ...
         CALL DASUDD ( HANDLE, BASE+DLCIDX, BASE+DLCIDX, DPLNK )
...
C
         CALL DASUDI ( HANDLE, BASE+ILCIDX, BASE+ILCIDX, NLINKS )
