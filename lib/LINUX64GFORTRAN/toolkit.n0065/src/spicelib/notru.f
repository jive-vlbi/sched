C$Procedure      NOTRU ( No true entries? )
 
      LOGICAL FUNCTION NOTRU ( LOGCLS, N )
 
C$ Abstract
C
C     Determine if none the entries in an array of logicals are .TRUE.
C
C$ Disclaimer
C
...
C
C     The function returns .TRUE. if no entry has a value of .TRUE.
C
C$ Detailed_Input
C
...
C$ Detailed_Output
C
C     The function returns true if no entry of LOGCLS is .TRUE.
C
C$ Parameters
C
C     None.
...
C     Error free.
C
C     If N is less than 1, the function returns a value of .TRUE.
C
C$ Files
C
C     None.
...
C     a .TRUE. value is found or until all values have been
C     examined.
C
C$ Examples
C
C     Suppose you needed to confirm that no entry of a character set
C     WORDS was one of the words in the phrase
C
...
C$ Index_Entries
C
C     test whether no logicals in an array are true
C
C-&
