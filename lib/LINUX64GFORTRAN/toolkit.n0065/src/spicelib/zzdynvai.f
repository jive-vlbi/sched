C$Procedure ZZDYNVAI ( Fetch array, integer frame kernel variable )

      SUBROUTINE ZZDYNVAI ( FRNAME, FRCODE, ITEM, MAXN, N, VALUES )
      IMPLICIT NONE
...
C
C     Look up array-valued integer frame kernel variable. The frame
...
C                    array-valued, integer kernel variable. The kernel
...
C
     ...
     .                 'frame # was expected to have integer '      //
     .                 'data type but in fact has '                 //
...
C
      CALL GIPOOL ( KVNAME, 1, MAXN, N, VALUES, FOUND )
