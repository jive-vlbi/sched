C$Procedure ZZDYNOAD ( Fetch optional array, d.p. frame variable )

      SUBROUTINE ZZDYNOAD ( FRNAME, FRCODE, ITEM,   
     .                      MAXN,   N,      VALUES, FOUND )
      IMPLICIT NONE
...
C
C     Look up optional array-valued double precision frame kernel
C     variable. The frame name or frame ID may be used as part of the
C     variable's name.
C
C     If the kernel variable is not present, or if the variable
C     has the wrong data type, set the FOUND flag to .FALSE. 
C
C$ Disclaimer
C
...
C                    array-valued, double precision kernel variable.
...
C                       FRAME_<frame name>_<ITEM>
C
C                    will be looked up.  If a numeric variable
...
C        variable names is found, but that variable has character data
...
C     is meant to look up array-valued double precision variables whose
...
C     data structure, these names are distinct.  Hence kernel
...
C$ Version
C
C-    SPICELIB Version 1.0.0, 16-DEC-2004 (NJB) 
C
C-&
...
C
     ...
     .                 'frame # was expected to have double '       //
     .                 'precision data type but in fact has  '      //
     .                 'character type.  Usually this type of '     //
...
C
      CALL GDPOOL ( KVNAME, 1, MAXN, N, VALUES, FOUND )
