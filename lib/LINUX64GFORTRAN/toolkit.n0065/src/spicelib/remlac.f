C$Procedure      REMLAC ( Remove elements from a character array )
 
      SUBROUTINE REMLAC ( NE, LOC, ARRAY, NA )
 
C$ Abstract
C
C      Remove one or more elements from a character array at the
...
C
C      CALL REMLAC ( 3,  1, ARRAY, -1 )
C      CALL REMLAC ( 3, -1, ARRAY,  7 )
C      CALL REMLAC ( 3,  6, ARRAY,  7 )
C
C$ Restrictions
C
C      None.
...
C$ Index_Entries
C
C     remove elements from a character array
C
C-&
