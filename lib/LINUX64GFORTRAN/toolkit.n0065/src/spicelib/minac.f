...
C$Procedure  MINAC  ( Minimum element of array, character )
 
      SUBROUTINE MINAC ( ARRAY, NDIM, MINVAL, LOC )
 
C$ Abstract
C
C     Locate the minimum element of a character array.
C
C$ Disclaimer
C
...
C
C     2) If the declared length of MINVAL is too short to contain the
...
C
C         CALL MINAC ( A, 6, MINVAL, LOC )
C
C      the values of MINVAL and LOC are 'Bardeen' and 5 respectively.
C
C$ Restrictions
C
C      None
...
C$ Index_Entries
C
C     minimum element of character array
C
C-&
