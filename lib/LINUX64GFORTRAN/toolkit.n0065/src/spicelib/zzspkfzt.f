C$Procedure ZZSPKFZT ( SPK function, easy reader, target )
 
      SUBROUTINE ZZSPKFZT ( TRGSUB, ET, REF, ABCORR, OBS, STARG, LT )
...
C
C                 The center of motion can be any ephemeris object; it
C                 need not be a gravity source.
C
...
C        ==============
C
C        ZZSPKFZT begins by computing the geometric position T(ET) of
...
C        for ABCORR, ZZSPKFZT computes the position of the target body
...
C        selected, ZZSPKFZT computes the position of the target body T
...
C$ Version
C
C-    SPICELIB Version 1.0.0, 04-JUL-2014 (CHA) (NJB) (WLT) (IMU)
C
...
C$ Index_Entries
C
C     target function easy reader for spk file
C
C-&
...
C        Add the state of the target's center to obtain the state
C        of the target with respect to the observer.
C 
         CALL TRGSUB ( ET, REF, TRGCTR, STTCTR )

         CALL SPKGEO ( TRGCTR, ET, REF, OBS, STEMP, LTCENT )
         ...
         CALL VADDG  ( STTCTR, STEMP, 6, STARG )
...
C        call ZZSPKFAT and return.
C
         IF ( TYPE .EQ. INERTL ) THEN
 
            CALL ZZSPKFAT ( TRGSUB, ET,    REF, ABCORR,
     .                      OBS,    STARG, LT,  DLT    )
            CALL CHKOUT ( RNAME )
            RETURN
...
C
         CALL ZZSPKFAT ( TRGSUB, ET,    'J2000', ABCORR,
     .                   OBS,    STATE, LT,      DLT    )
...
C
C        If the frame is centered at the observer, the light time and
C        light time rate are both zero. If the frame is centered
C        elsewhere, we'll need to obtain the light time between the
C        observer and the frame center. Unlike SPKEZ, we have no case
C        here for the frame centered at the target, since the target
C        isn't an ephemeris object, as far as this routine can
C        determine.
C
