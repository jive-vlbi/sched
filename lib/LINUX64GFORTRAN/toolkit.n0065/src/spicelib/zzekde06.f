C$Procedure      ZZEKDE06 ( EK, delete column entry, class 6 )
 
      SUBROUTINE ZZEKDE06 ( HANDLE, SEGDSC, COLDSC, RECPTR )
 
C$ Abstract
C
C     Delete a specified class 6 column entry from an EK record.
C
C$ Disclaimer
C
...
C
      ...
      IF ( DATPTR .GT. 0 ) THEN
C
C        Get the element count for the entry.  Compute the character
C        count for the entry.
C
         CALL ZZEKGEI ( HANDLE, DATPTR, NELTS )
...
C
         CALL ZZEKGFWD ( HANDLE, CHR, P, NEXT )
...
C
            CALL ZZEKDPS  ( HANDLE, SEGDSC, CHR, P )
 
         END IF
 
 
         NSEEN = MIN (  NCHARS,  (BASE+CPSIZE-DATPTR)  )
 
 
         DO WHILE (  ( NSEEN .LT. NCHARS ) .AND. ( .NOT. FAILED() )  )
C
