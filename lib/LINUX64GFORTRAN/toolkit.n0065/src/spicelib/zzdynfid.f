C$Procedure ZZDYNFID ( Fetch frame ID kernel variable )

      SUBROUTINE ZZDYNFID ( FRNAME, FRCODE, ITEM, IDCODE )
      IMPLICIT NONE 
...
C
C     Look up a frame definition kernel variable whose associated
C     value is a frame name or frame ID code.  The returned value is
C     always an ID code.  The kernel variable name can refer to 
C     the frame being defined by either name or ID code.
C
C     If the kernel variable is not present, or if the variable
C     is not a frame name or a numeric value, signal an error.
C
C$ Disclaimer
C
...
C     IDCODE     O   Output kernel variable.
C
C$ Detailed_Input
C
C     FRNAME         is the name of the reference frame with which
C                    the requested variable is associated.  This frame
C                    may be thought of as the frame associated with
C                    "left hand side" of the kernel variable 
C                    assignment.
C
C     FRCODE         is the frame ID code of the reference frame with
C                    which the requested variable is associated.  This
C                    is the ID code corresponding to FRNAME.
C
...
C                    must be one of
C
C                       - a reference frame ID code 
C 
C                       - a string representation of an integer,
C                         for example '5'
C
C                       - a reference frame name
C
C$ Detailed_Output
C
C     IDCODE         is the frame ID code associated with the value of
C                    the requested kernel variable. This frame may be
C                    regarded as being associated with the "right hand
C                    side." of the kernel variable assignment. The
...
C                    be converted to a frame ID code, and that
...
C                    frame ID code, and that code will be returned.
C
...
C        variable names is found, but that variable has more than one
...
C        frame ID code or an integer, the error SPICE(NOTRANSLATION) 
C        is signaled.
C
...
C        frames associated with velocity vectors or constant vectors
C        serving to define two-vector dynamic frames.
C
C$ Restrictions
C
...
C
         CALL GCPOOL ( KVNAME, 1, 1, N, OUTNAM, FOUND )     

         IF ( .NOT. FOUND ) THEN

            CALL SETMSG ( 'The kernel variable # has used to define ' //
     .                    'frame # was not found after DTPOOL '       //
...
C
C        Convert the output frame name to a frame code.
C
         CALL NAMFRM ( OUTNAM, IDCODE )

         IF ( IDCODE .EQ. 0 ) THEN
C
C           If IDCODE is zero, that means NAMFRM couldn't translate
C           the name.  Perhaps the name is an integer?
C
            IF ( BEINT(OUTNAM) ) THEN

               CALL PRSINT ( OUTNAM, IDCODE ) 

            ELSE
C
C              We're outta aces.
C
               CALL SETMSG ( 'The kernel variable # used to define '  //
     .                       'frame # is assigned the character '     //
     .                       'value #.  This value was expected to '  //
     .                       'be a reference frame name, but NAMFRM ' //
     .                       'cannot translate this name to a frame ' //
     .                       'ID code.'                               )
               CALL ERRCH  ( '#',  KVNAME                             )
               CALL ERRCH  ( '#',  FRNAME                             )
               CALL ERRCH  ( '#',  OUTNAM                             )
         ...
         END IF
C
C        IDCODE has been assigned a value at this point.
C
