...
C      BSRCHC     O   Index of VALUE in ARRAY. (Zero if not found.)
C
C$ Detailed_Input
C
...
C                  ARRAY are assumed to sorted according to the
C                  ASCII collating sequence.
C
C$ Detailed_Output
C
C      BSRCHC      is the index of the input value in the input array.
C                  If ARRAY does not contain VALUE, BSRCHC is zero.
C
C                  If ARRAY contains more than one occurrence of VALUE,
C                  BSRCHC may point to any of the occurrences.
C
C$ Parameters
C
C     None.
...
C$ Restrictions
C
C      ARRAY is assumed to be sorted in increasing order according to
