C$Procedure ZZEKLLED ( EK, last less than or equal to, d.p. )
 
      SUBROUTINE ZZEKLLED (  HANDLE,  SEGDSC,  COLDSC,
     .                       DKEY,    PRVLOC,  PRVPTR  )
...
C     PRVPTR     O   Record pointer for predecessor of DKEY.
C
C$ Detailed_Input
C
...
C                    searched.
C
C     DKEY           is an double precision string key.  The last column
C                    entry less than this key is sought.
C
C$ Detailed_Output
C
...
C                    equal to DKEY.  If the column contains elements
C                    equal to DKEY, PRVLOC is the index of the last
C                    such element.
C
C                    If all elements of the column are greater than
C                    DKEY, PRVLOC is set to zero.
C
...
C     to a specified double precision key, within a specified segment
C     and column.
C
