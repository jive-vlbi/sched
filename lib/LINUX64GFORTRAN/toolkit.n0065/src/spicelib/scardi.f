C$Procedure      SCARDI ( Set the cardinality of an integer cell )
 
      SUBROUTINE SCARDI ( CARD, CELL )
 
C$ Abstract
C
C     Set the cardinality of an integer cell.
C
C$ Disclaimer
C
...
C
      IF (  ( CARD .LT. 0 )  .OR.  ( CARD .GT. CELL(-1) )  )  THEN
