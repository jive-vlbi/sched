C$Procedure      ZZEKUE01 ( EK, update column entry, class 1 )
 
      SUBROUTINE ZZEKUE01 (  HANDLE,  SEGDSC,  COLDSC,
     .                       RECPTR,  IVAL,    ISNULL  )
 
C$ Abstract
C
C     Update a specified class 1 column entry in an EK record.
C
C$ Disclaimer
C
...
C                    entry to update.
C
C     IVAL           is the integer value with which to update the
C                    specified column entry.
C
C
...
C
            ...
            CALL ZZEKIII1 ( HANDLE,  SEGDSC,  COLDSC,  IVAL,
     .                      RECPTR,  ISNULL                   )
...
C
            CALL ZZEKPGPG ( INT,    DATPTR, P,      PBASE    )
...
C
            ...
            CALL ZZEKAD01 (  HANDLE,  SEGDSC,  COLDSC,
     .                       RECPTR,  IVAL,    ISNULL  )
...
C
         CALL ZZEKAD01 ( HANDLE, SEGDSC, COLDSC, RECPTR, IVAL, ISNULL )
