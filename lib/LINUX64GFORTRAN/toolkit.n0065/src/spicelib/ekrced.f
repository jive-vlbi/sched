C$Procedure   EKRCED ( EK, read column entry element, d.p. )
...
C$ Abstract
C
C     Read data from a double precision column in a specified EK
C     record.
C
C$ Disclaimer
C
...
C     DVALS      O   D.p. values in column entry.
...
C
C     NVALS,
C     DVALS          are, respectively, the number of values found in
...
C$ Examples
C
C     1)  Read the value in the third record of the column DCOL in
C         the fifth segment of an EK file designated by HANDLE.
C
C            CALL EKRCED ( HANDLE, 5, 3, 'DCOL', N, DVAL, ISNULL )
C
C$ Restrictions
C
...
C        to look up data in the class 8 case.  Miscellaneous header
...
C$ Index_Entries
C
C     read double precision data from EK column
C
C-&
...
C
C        Bug fix:  Record number, not record pointer, is now supplied
C        to look up data in the class 8 case.  For class 8 columns,
...
C
      ...
      IF ( ( DTYPE .NE. DP ) .AND. ( DTYPE .NE. TIME )  ) THEN
         ...
         CALL SETMSG ( 'Column # is of type #; EKRCED only works '    //
...
C
         ...
         CALL ZZEKRD02 ( HANDLE, SEGDSC, COLDSC, RECPTR, DVALS, ISNULL )
         NVALS  =  1
     ...
     .                    1,       NVALS,   DVALS,   ISNULL,  FOUND  )
 
 
      ELSE IF ( CLASS .EQ. 8 ) THEN
C
C        Records in class 8 columns are identified by a record number
C        rather than a pointer.
C 
         CALL ZZEKRD08 ( HANDLE, SEGDSC, COLDSC, RECNO, DVALS, ISNULL )
         NVALS  =  1
