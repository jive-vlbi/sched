C$Procedure      DIFFC ( Difference of two character sets )
 
      SUBROUTINE DIFFC ( A, B, C )
 
C$ Abstract
C
C      Take the difference of two character sets to form a third set.
C
C$ Disclaimer
C
...
C
C      2) If length of the elements of the output set is less than
C         the length of the elements of the FIRST input set, the
C         error SPICE(ELEMENTSTOOSHORT) is signalled.
C
C$ Files
C
C      None.
...
C$ Index_Entries
C
C     difference of two character sets
C
C-&
...
C
      IF (   LEN( C(LBCELL) )  .LT.   LEN( A(LBCELL) )  ) THEN
