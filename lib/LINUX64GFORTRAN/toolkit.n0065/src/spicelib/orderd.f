C$Procedure      ORDERD ( Order of a double precision array )
...
C$ Abstract
C
C     Determine the order of elements in a double precision array.
C
C$ Disclaimer
C
...
C$ Particulars
C
C     ORDERD finds the index of the smallest element of the input
C     array. This becomes the first element of the order vector.
C     The process is repeated for the rest of the elements.
C
C     The order vector returned by ORDERD may be used by any of
...
C
C            C
C            C     Sort the object arrays by visual magnitude.
C            C
C                  CALL ORDERD ( VMAGS,  N, IORDER )
C
