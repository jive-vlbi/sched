C$Procedure ZZRVAR ( Private --- Pool, read the next kernel variable )
 
      SUBROUTINE ZZRVAR ( NAMLST,
     .                    NMPOOL, NAMES, DATLST,
     .                    DPPOOL, DPVALS,
     .                    CHPOOL, CHVALS,
     .                    VARNAM, EOF )
...
C
C     Read the next variable from a SPICE ASCII kernel file into
C     the kernel pool.
C
C$ Disclaimer
C
...
C     NMPOOL     include the next variable read from the current
C     NAMES      active text kernel in RDKER.
C     DATLST
C     DPPOOL
C     DPVALS
C     CHPOOL
...
C
C
C     1) The error 'SPICE(BADTIMESPEC)' is signaled if a value
C        beginning with '@' cannot be parsed as a time.
C
C     2) The error 'SPICE(BADVARASSIGN)' is signaled if variable
C        assignment does not have the form NAME = [(] value [ value ) ].
C
C     3) The error 'SPICE(KERNELPOOLFULL)' is signaled if there is
C        no room left in the kernel pool to store another variable
C        or value.
C
C     4) The error 'SPICE(NONPRINTINGCHAR)' is signaled if the name
C        in a variable assignment contains a non-printing character.
C
C     5) The error 'SPICE(NUMBEREXPECTED)' is signaled if a value
...
C$ Files
C
C     ZZRVAR reads from the file most recently opened by RDKNEW.
C
C$ Particulars
C
C     None.
...
C$ Version
C
C-    SPICELIB Version 1.7.0, 08-FEB-2010 (EDW)
C
C        Added an error check on the length of the kernel pool variable
C        name read from the kernel file.
C
C-    SPICELIB Version 1.6.0, 06-AUG-2002 (BVS)
C 
C        Modified to make sure that DO WHILE loop that looks for the 
C        end of string variable value always exits.
C
C-    SPICELIB Version 1.5.0, 07-APR-2000 (WLT)
C 
C        Happy Birthday Alex. Added check to the assignment to CHVALS
C        so that we cannot store data past the end of the string.
C
C-    SPICELIB Version 1.4.0, 22-MAR-1999 (WLT)
C
C        Added code to detect and signal an error for empty
C        vector assignment.
C
C-    SPICELIB Version 1.3.0, 16-JAN-1997 (WLT)
C
C        The error message regarding the directives allowed
C        in a keyword =  value directive was updated.
C
C-    SPICELIB Version 1.1.0, 25-JUN-1996 (WLT)
C
C        The error message for unparsed numeric components
C        was corrected so that it now shows the line and
C        line number on which the error occurred.
C
...
C
            ...
            IF ( VARLEN .GT. MAXLEN ) THEN
               ...
               CALL SETMSG ( 'A kernel pool variable name read from a '
     .         //            'kernel file exceeds the maximum allowed '
     .         //            'length #1. The actual length of the '
     .         //            'variable name is #2, the offending '
     .         //            'variable name to #3 characters: '
     .         //            '''#4''.')
...
C
     ...
     .            //            '# of the kernel file ''#'' is '
     .            //            'not a string value. ' )
 
                  CALL ERRCH  ( '#', VARNAM(1:R1) )
                  CALL ERRINT ( '#', NUMBER       )
                  CALL ERRCH  ( '#', FILE  (1:R2) )
...
C
C              Adjust end-of-token position (E) if it happens to the
C              last, non-quote character of the truncated input line.
C              This has to be done to make sure that all meaningful
C              characters get moved to the value.
C
               CODE = ICHAR(LINE(E:E))
               IF ( .NOT. ISQUOT(CODE) ) THEN
                  E = E + 1
               END IF
               ...
               DO WHILE ( J .LT. E )
                  ...
                  IF ( I .LE. LEN(CHVALS(CHNODE)) ) THEN
