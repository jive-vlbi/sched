...
C$Procedure ZZSPKSB1 ( S/P Kernel, solar system barycenter )
 
      SUBROUTINE ZZSPKSB1 ( TARG, ET, REF, STARG )
...
C     ZZSPKSB1 reads as many segments as necessary, from as many
C     files as necessary, using files that have been loaded by
C     previous calls to SPKLEF (load ephemeris file).
C
C$ Examples
C
C     In the following code fragment, ZZSPKSB1 is used to display
...
C           CALL ZZSPKSB1 ( EARTH, EPOCH, 'J2000', SEARTH )
C           CALL ZZSPKSB1 ( MARS,  EPOCH, 'J2000', SMARS  )
C
...
C
C     2) The ephemeris files to be used by ZZSPKSB1 must be loaded
C        by SPKLEF before ZZSPKSB1 is called.
C
...
C
      ...
      CALL ZZSPKGO1 ( TARG, ET, REF, BARY, STARG, LT )
 
      CALL CHKOUT ( 'ZZSPKSB1' )
      RETURN
      END
