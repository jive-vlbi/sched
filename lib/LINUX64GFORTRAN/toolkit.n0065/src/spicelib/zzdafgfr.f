C$Procedure ZZDAFGFR ( Private --- DAF Get Data Record )
 
      SUBROUTINE ZZDAFGFR ( HANDLE, IDWORD, ND,   NI,   IFNAME,
     .                      FWARD,  BWARD,  FREE, FOUND         )
 
C$ Abstract
C
C     SPICE Private routine intended solely for the support of SPICE
C     routines.  Users should not call this routine directly due
C     to the volatile nature of this routine.
C
C     Read the contents of the file record of a DAF.
C
C$ Disclaimer
C
C     THIS SOFTWARE AND ANY RELATED MATERIALS WERE CREATED BY THE
C     CALIFORNIA INSTITUTE OF TECHNOLOGY (CALTECH) UNDER A U.S.
C     GOVERNMENT CONTRACT WITH THE NATIONAL AERONAUTICS AND SPACE
C     ADMINISTRATION (NASA). THE SOFTWARE IS TECHNOLOGY AND SOFTWARE
C     PUBLICLY AVAILABLE UNDER U.S. EXPORT LAWS AND IS PROVIDED "AS-IS"
C     TO THE RECIPIENT WITHOUT WARRANTY OF ANY KIND, INCLUDING ANY
C     WARRANTIES OF PERFORMANCE OR MERCHANTABILITY OR FITNESS FOR A
C     PARTICULAR USE OR PURPOSE (AS SET FORTH IN UNITED STATES UCC
C     SECTIONS 2312-2313) OR FOR ANY PURPOSE WHATSOEVER, FOR THE
C     SOFTWARE AND RELATED MATERIALS, HOWEVER USED.
C
C     IN NO EVENT SHALL CALTECH, ITS JET PROPULSION LABORATORY, OR NASA
C     BE LIABLE FOR ANY DAMAGES AND/OR COSTS, INCLUDING, BUT NOT
C     LIMITED TO, INCIDENTAL OR CONSEQUENTIAL DAMAGES OF ANY KIND,
C     INCLUDING ECONOMIC DAMAGE OR INJURY TO PROPERTY AND LOST PROFITS,
C     REGARDLESS OF WHETHER CALTECH, JPL, OR NASA BE ADVISED, HAVE
C     REASON TO KNOW, OR, IN FACT, SHALL KNOW OF THE POSSIBILITY.
C
C     RECIPIENT BEARS ALL RISK RELATING TO QUALITY AND PERFORMANCE OF
C     THE SOFTWARE AND ANY RELATED MATERIALS, AND AGREES TO INDEMNIFY
C     CALTECH AND NASA FOR ALL THIRD-PARTY CLAIMS RESULTING FROM THE
C     ACTIONS OF RECIPIENT IN THE USE OF THE SOFTWARE.
C
C$ Required_Reading
C
C     None.
C
C$ Keywords
C
C     PRIVATE
C
C$ Declarations
 
      IMPLICIT NONE
 
      INCLUDE              'zzddhman.inc'
 
      INTEGER               HANDLE
      CHARACTER*(*)         IDWORD
      INTEGER               ND
      INTEGER               NI
      CHARACTER*(*)         IFNAME
      INTEGER               FWARD
      INTEGER               BWARD
      INTEGER               FREE
      LOGICAL               FOUND
 
C$ Brief_I/O
C
C     VARIABLE  I/O  DESCRIPTION
C     --------  ---  --------------------------------------------------
C     HANDLE     I   Handle of the DAF.
C     IDWORD     O   DAF ID Word that indicates file type.
C     ND         O   Number of double precision components in summaries.
C     NI         O   Number of integer components in summaries.
C     IFNAME     O   Internal file name.
C     FWARD      O   Forward list pointer.
C     BWARD      O   Backward list pointer.
C     FREE       O   Free address pointer.
C     FOUND      O   Logical indicating whether the record was found.
C
C$ Detailed_Input
C
C     HANDLE     is the handle associated with the DAF.
C
C$ Detailed_Output
C
C     IDWORD     is a character string identifying the architecture
C                and type of a SPICE binary kernel.  In this case
C                it will be a string identifying the type of DAF.
C
C     ND,
C     NI         are the number of double precision and integer
C                components, respectively, in each array summary in
C                the specified file.
C
C     IFNAME     is the internal file name stored in the first
C                (or file) record of the specified file.
C
C     FWARD      is the forward list pointer. This points to the
C                first summary record in the file. (Records between
C                the first record and the first summary record are
C                reserved when the file is created, and are invisible
C                to DAF routines.)
C
C     BWARD      is the backward list pointer. This points
C                to the final summary record in the file.
C
C     FREE       is the free address pointer. This contains the
C                first free address in the file. (That is, the
C                initial address of the next array to be added
C                to the file.)
C
C     FOUND      is TRUE when the file record is found, and is
C                FALSE otherwise.
C
C$ Parameters
C
C     None.
C
C$ Files
C
C     This routine reads data from the DAF associated with HANDLE.
C     This action may result in connecting a logical unit to the
C     file, if the handle manager has rotated the file out of the
C     unit table.
C
C$ Exceptions
C
C     1) SPICE(HANDLENOTFOUND) is signaled if HANDLE can not be
C        found in the set of loaded handles.  The output arguments
C        are unmodified when this error occurs.
C
C     2) Routines in the call tree of this routine may trap and
C        signal errors.  The output arguments are unmodified in
C        these cases.
C
C$ Particulars
C
C     This routine reads the publically available components of
C     file records from native and supported non-native DAFs.
C
C     The size of the character buffer and the number of records
C     read may have to change to support new environments.  As of
C     the original release of this routine, all systems currently
C     supported have a 1 kilobyte record length.
C
C$ Examples
C
C     See DAFRFR for sample usage.
C
C$ Restrictions
C
C     1) Numeric data when read as characters from a file preserves
C        the bit patterns present in the file in memory.
C
C     2) A record of double precision data is at most 1024 characters
C        in length.
C
C     3) Future updates to this module must preserve the fact that
C        FOUND is returned as FALSE whenever an error occurs.  An
C        incompletely translated or extracted file record is NOT
C        FOUND.
C
C$ Author_and_Institution
C
C     F.S. Turner     (JPL)
C
C$ Literature_References
C
C     None.
C
C$ Version
C
C-    SPICELIB Version 1.0.0, 12-NOV-2001 (FST)
C
C
C-&
 
C
C     SPICELIB Functions
C
      INTEGER               ISRCHC
 
      LOGICAL               FAILED
      LOGICAL               RETURN
 
C
C     Local Parameters
C
C     Record Number of the file record in a DAF.
C
      INTEGER               FRLOCR
      PARAMETER           ( FRLOCR =  1 )
 
C
C     Length of the IDWORD string.
C
      INTEGER               IDWLEN
      PARAMETER           ( IDWLEN =  8 )
 
C
C     Length of the internal filename string.
C
      INTEGER               IFNLEN
      PARAMETER           ( IFNLEN = 60 )
 
C
C     Starting location in bytes of the internal filename in the
C     file record.
C
      INTEGER               IFNSTR
      PARAMETER           ( IFNSTR = 17 )
 
C
C     Size of an integer in bytes.
C
      INTEGER               INLEN
      PARAMETER           ( INLEN  =  4 )
 
C
C     Local Variables
C
      CHARACTER*(CBFSIZ)    CHRBUF
      CHARACTER*(FILEN)     FNAME
      CHARACTER*(IDWLEN)    LOCIDW
      CHARACTER*(IFNLEN)    LOCIFN
      CHARACTER*(STRSIZ)    STRBFF ( NUMBFF )
      CHARACTER*(STRSIZ)    TMPSTR
 
      INTEGER               CINDEX
      INTEGER               I
      INTEGER               IAMH
      INTEGER               IARCH
      INTEGER               IBFF
      INTEGER               IOSTAT
      INTEGER               LOCBWD
      INTEGER               LOCFRE
      INTEGER               LOCFWD
      INTEGER               LOCND
      INTEGER               LOCNI
      INTEGER               LUN
      INTEGER               NATBFF
 
      LOGICAL               FIRST
      LOGICAL               LOCFND
 
C
C     Saved Variables
C
      SAVE                  FIRST
      SAVE                  NATBFF
      SAVE                  STRBFF
 
C
C     Data Statements
C
      DATA                  FIRST  / .TRUE. /
      DATA                  NATBFF / 0      /
 
C
C     Standard SPICE error handling.
C
      IF ( RETURN() ) THEN
         RETURN
      ELSE
         CALL CHKIN ( 'ZZDAFGFR' )
      END IF
 
C
C     Perform some initialization tasks.
C
      IF ( FIRST ) THEN
 
C
C        Populate STRBFF, the buffer that contains the labels
C        for each binary file format.
C
         DO I = 1, NUMBFF
            CALL ZZDDHGSD ( 'BFF', I, STRBFF(I) )
         END DO
 
C
C        Fetch the native binary file format and determine its
C        integer code.
C
         CALL ZZPLATFM ( 'FILE_FORMAT', TMPSTR )
         CALL UCASE    ( TMPSTR,        TMPSTR )
 
         NATBFF = ISRCHC ( TMPSTR, NUMBFF, STRBFF )
 
         IF ( NATBFF .EQ. 0 ) THEN
 
            CALL SETMSG ( 'The binary file format, ''#'', is not '
     .      //            'supported by this version of the toolkit. '
     .      //            'This is a serious problem, contact NAIF.'   )
            CALL ERRCH  ( '#', TMPSTR                                  )
            CALL SIGERR ( 'SPICE(BUG)'                                 )
            CALL CHKOUT ( 'ZZDAFGFR'                                   )
            RETURN
 
         END IF
 
C
C        Do not perform initialization tasks again.
C
         FIRST = .FALSE.
 
      END IF
 
C
C     Assume the data record will not be found, until it has been read
C     from the file, and if necessary, successfully translated.
C
      FOUND = .FALSE.
 
C
C     Retrieve information regarding the file from the handle manager.
C     The value of IARCH is not a concern, since this is a DAF routine
C     all values passed into handle manager entry points will have
C     'DAF' as their architecture arguments.
C
      CALL ZZDDHNFO ( HANDLE, FNAME, IARCH, IBFF, IAMH, LOCFND )
 
      IF ( .NOT. LOCFND ) THEN
         CALL SETMSG ( 'Unable to locate file associated with '
     .   //            'HANDLE, #.  The most likely cause of this '
     .   //            'is the file that you are trying to read '
     .   //            'has been closed.'                           )
         CALL ERRINT ( '#', HANDLE                                  )
         CALL SIGERR ( 'SPICE(HANDLENOTFOUND)'                      )
         CALL CHKOUT ( 'ZZDAFGFR'                                   )
         RETURN
      END IF
 
C
C     Now get a logical unit for the handle.  Check FAILED() in
C     case an error occurs.
C
      CALL ZZDDHHLU ( HANDLE, 'DAF', .FALSE., LUN )
 
      IF ( FAILED() ) THEN
         CALL CHKOUT ( 'ZZDAFGFR' )
         RETURN
      END IF
 
C
C     Branch based on whether the binary file format is native
C     or not.  Only supported formats can be opened by ZZDDHOPN,
C     so no check of IBFF is required.
C
      IF ( IBFF .EQ. NATBFF ) THEN
 
C
C        In the native case, just read the components of the file
C        record from the file.
C
         READ ( UNIT   = LUN,
     .          REC    = FRLOCR,
     .          IOSTAT = IOSTAT  ) LOCIDW, LOCND,  LOCNI, LOCIFN,
     .                             LOCFWD, LOCBWD, LOCFRE
 
C
C        Since this routine does not signal any IOSTAT based
C        errors, return if a non-zero value is assigned to IOSTAT.
C
         IF ( IOSTAT .NE. 0 ) THEN
            CALL CHKOUT ( 'ZZDAFGFR' )
            RETURN
         END IF
 
C
C     Process the non-native binary file format case.
C
      ELSE
 
C
C        Read the data record as characters.
C
         READ ( UNIT   = LUN,
     .          REC    = FRLOCR,
     .          IOSTAT = IOSTAT  ) CHRBUF
 
C
C        Again, since this routine does not signal any IOSTAT
C        based errors, return if one occurs.
C
         IF ( IOSTAT .NE. 0 ) THEN
            CALL CHKOUT ( 'ZZDAFGFR' )
            RETURN
         END IF
 
C
C        Assign the character components of the file record.
C
         LOCIDW = CHRBUF( 1      : IDWLEN            )
         LOCIFN = CHRBUF( IFNSTR : (IFNSTR+IFNLEN-1) )
 
C
C        Convert the integer components.
C
         CINDEX = IDWLEN + 1
         CALL ZZXLATEI ( IBFF, CHRBUF(CINDEX:CINDEX+INLEN-1),
     .                   1,    LOCND                          )
 
         CINDEX = CINDEX + INLEN
         CALL ZZXLATEI ( IBFF, CHRBUF(CINDEX:CINDEX+INLEN-1),
     .                   1,    LOCNI                          )
 
         CINDEX = IFNSTR + IFNLEN
         CALL ZZXLATEI ( IBFF, CHRBUF(CINDEX:CINDEX+INLEN-1),
     .                   1,    LOCFWD                         )
 
         CINDEX = CINDEX + INLEN
         CALL ZZXLATEI ( IBFF, CHRBUF(CINDEX:CINDEX+INLEN-1),
     .                   1,    LOCBWD                         )
 
         CINDEX = CINDEX + INLEN
         CALL ZZXLATEI ( IBFF, CHRBUF(CINDEX:CINDEX+INLEN-1),
     .                   1,    LOCFRE                         )
 
         IF ( FAILED() ) THEN
            CALL CHKOUT ( 'ZZDAFGFR' )
            RETURN
         END IF
 
      END IF
 
C
C     Transfer the contents of the record to the output arguments
C     and return to the caller.
C
      FOUND = .TRUE.
 
      IDWORD = LOCIDW
      ND     = LOCND
      NI     = LOCNI
      IFNAME = LOCIFN
      FWARD  = LOCFWD
      BWARD  = LOCBWD
      FREE   = LOCFRE
 
      CALL CHKOUT( 'ZZDAFGFR' )
      RETURN
 
      END
