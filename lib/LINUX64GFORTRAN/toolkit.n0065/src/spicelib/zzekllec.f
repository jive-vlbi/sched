C$Procedure ZZEKLLEC ( EK, last less than or equal to, character )
 
      SUBROUTINE ZZEKLLEC (  HANDLE,  SEGDSC,  COLDSC,
     .                       CKEY,    PRVLOC,  PRVPTR  )
...
C                    equal to CKEY.  If the column contains elements
C                    equal to CKEY, PRVLOC is the index of the last
C                    such element.
C
C                    If all elements of the column are greater than
C                    CKEY, PRVLOC is set to zero.
C
...
C
C     Get the record pointers BEGPTR and ENDPTR to the least and
