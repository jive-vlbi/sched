C$Procedure      SWAPAI ( Swap elements within an integer array )
 
      SUBROUTINE SWAPAI ( N, LOCN, M, LOCM, ARRAY )
 
C$ Abstract
C
C     Swap (exchange) two non-intersecting groups of contiguous
C     elements of an integer array.
C
C$ Disclaimer
C
...
C$ Index_Entries
C
C     swap elements within an integer array
C
C-&
...
C
      ...
      DO I = 0, DIRECT-1
         CALL SWAPI ( ARRAY(LN+I), ARRAY(LM+I) )
      END DO
...
C
      ...
      IF ( EXTRA .GT. 0 ) THEN
         ...
         CALL CYAIIP ( NSUB, DIR, EXTRA, ARRAY(BEGSUB) )
