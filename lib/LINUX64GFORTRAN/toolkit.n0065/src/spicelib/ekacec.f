...
C$ Examples
C
C     1)  Add the value '999' to the third record of the column CCOL in
C         the fifth segment of an EK file designated by HANDLE.
C
C            CALL EKACEC ( HANDLE, 5, 3, 'CCOL', 1, '999', .FALSE. )
C
C
C     2)  Same as (1), but this time add a null value.  The argument
C         999 is ignored because the null flag is set to .TRUE.
C
C            CALL EKACEC ( HANDLE, 5, 3, 'CCOL', 1, '999', .TRUE. )
C
C
C     3)  Add an array CBUFF of 10 values to the third record of the
C         column CARRAY in the fifth segment of an EK file designated by
C         HANDLE.
C
C            CALL EKACEC ( HANDLE, 5, 3, 'CARRAY', 10, CBUFF, .FALSE. )
C
C
