C$Procedure      PODONC ( Pod, offset and number, character )
 
      SUBROUTINE PODONC ( POD, OFFSET, NUMBER )
      IMPLICIT NONE
...
C
      CALL     DCODEC ( POD(GRPOFF),   OFFSET )
      NUMBER = CARDC  ( POD       )  - OFFSET
