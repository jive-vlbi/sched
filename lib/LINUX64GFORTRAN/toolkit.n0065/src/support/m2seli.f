C$Procedure M2SELI ( META/2 --- select a named integer )
 
      SUBROUTINE M2SELI ( NAME, STRING, NTH, FOUND, INT  )
      IMPLICIT NONE
 
C$ Abstract
C
C     Select the Nth substring associated with a matched, named META/2
C     template word and parse it as an integer.
C
C$ Disclaimer
C
...
C
C     INT        is the integer represented by the word of STRING that
...
C
     ...
     .                 'as an integer. a number.'                     )
         CALL SIGERR ( 'META/2(CORRUPTEDINTEGER)'                     )
         CALL CHKOUT ( 'M2SELI' )
         RETURN
      END IF
