C$Procedure      M2DAY ( Determine whether or not a word is a day )
 
      LOGICAL FUNCTION M2DAY ( WORD )
      IMPLICIT NONE
 
C$ Abstract
C
C     This function is true if the input string is a day in the
C     sense of META/2.
C
C$ Disclaimer
C
...
C
C     The function is returned as .TRUE. if word is a META/2 day.
C
C$ Detailed_Input
C
...
C$ Detailed_Output
C
C     M2DAY     returns as .TRUE. if WORD is a META/2 day.
...
C     determines whether or not a word is a day in the sense
...
C
      ...
      ELSE IF ( LENGTH .EQ. 3 ) THEN
         ...
         I2    =  I1 + 1
         VALUE =  100  * VALUES( ICHAR( WORD(I1:I1) ) )
     .         +   10  * VALUES( ICHAR( WORD(I2:I2) ) )
     .         +         VALUES( ICHAR( WORD(I3:I3) ) )
     .
 
      ELSE IF ( LENGTH .EQ. 2 ) THEN
 
         VALUE =  10   * VALUES( ICHAR( WORD(I1:I1) ) )
     .         +         VALUES( ICHAR( WORD(I3:I3) ) )
      ELSE
 
         VALUE =         VALUES( ICHAR( WORD(I1:I1) ) )
 
      END IF
C
C     That's all just make sure that the value is within the
C     bound required of a day of month or year.
C
      M2DAY = (   VALUE .GE. 1 ) .AND. ( VALUE .LE. 366 )
