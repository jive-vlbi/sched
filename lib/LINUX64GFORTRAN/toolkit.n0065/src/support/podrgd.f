C$Procedure      PODRGD ( Pod, replace group, double precision )
 
      SUBROUTINE PODRGD ( POD )
      IMPLICIT NONE
 
C$ Abstract
C
C     End the active group of a pod, replacing the previous group.
C
C$ Disclaimer
C
...
C     move that offset into element GRPOFF. To replace the previous
C     group, just move all of the elements of the active group
C     toward the front of the pod. The new cardinality, of course,
C     should be the new offset plus the number of elements in the
C     original group.
C
...
C
      ...
      IF ( OFFSET .NE. 0 ) THEN
         ...
         CALL DCODED ( POD(OFFSET), NEWOFF )
 
         DO I = 1, NUMBER
            POD(NEWOFF+I) = POD(OFFSET+I)
         END DO
 
         CALL SCARDD ( NEWOFF + NUMBER, POD )
