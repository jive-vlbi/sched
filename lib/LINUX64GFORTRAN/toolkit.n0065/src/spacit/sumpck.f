C$Procedure      SUMPCK ( Summarize an PCK file )

      SUBROUTINE SUMPCK ( HANDLE, BINFNM, LPSFNM, LOGFIL, LOGLUN )

C$ Abstract
C
C     Summarize an PCK file.
C
C$ Disclaimer
C
...
C     --------  ---  --------------------------------------------------
C     HANDLE     I   Handle of the PCK file to be summarized.
C     LOGFIL     I   Write the summary to a log file and to screen?
C     LOGLUN     I   Logical unit connected to the log file.
C     NDC        P   Number of d.p. components in PCK descriptor.
C     NIC        P   Number of integer components in PCK descriptor.
C     NC         P   Size of packed PCK descriptor.
C     IDSIZ      P   Length of PCK segment identifier.
C     LBCELL     P   Lower bound for the SPICELIB CELL data structure.
C
C$ Detailed_Input
C
C     HANDLE     is the integer handle of the PCK file to be summarized.
C
...
C                unpacked PCK descriptor.
C
C     NIC        is the number of integer components in an unpacked
C                PCK descriptor.
C
C     NC         is the size of a packed PCK descriptor.
C
C     IDSIZ      is the length of an PCK segment identifier.
C
...
C$ Files
C
C     The PCK file to be summarized is referred throughout this routine
...
C
C
C-    Beta Version 2.0.0  21-JUL-1995 (KRG)
C
...
C$ Index_Entries
C
C      summarize the segments in a binary pck file
C
C-&
...
C
      CHARACTER*(*)         FNMLBL
      PARAMETER           ( FNMLBL = 'Summary for PCK file: #' )
...
C
      DATA   MENUTL    / 'PCK Summary Options' /
C
...
C
C              Summarize the entire file. Write the type
C              of summary to the log file if we need to.
C
C
...
C
                  CALL PCKGSS ( SEGID,  SEGBOD, SEGFRM, SEGTYP, 
...
C
                     ...
                     CALL PCKWSS ( STDOUT, SEGID,  SEGBOD, 
     .                             SEGFRM, SEGTYP, SEGBTM, 
     .                             SEGETM                  )

                     IF ( LOGFIL ) THEN
                        CALL PCKWSS ( LOGLUN, SEGID,  SEGBOD, 
     .                                SEGFRM, SEGTYP, SEGBTM, 
     .                                SEGETM                  )
                     END IF
