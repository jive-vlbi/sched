C$Procedure      SYDELD ( Delete a symbol from a symbol table )
 
      SUBROUTINE SYDELD ( NAME, TABSYM, TABPTR, TABVAL )
 
C$ Abstract
C
C     Delete a symbol from a double precision symbol table. The symbol
...
C$ Examples
C
C     In the following example the subroutine SYDELD is used to delete
C     the symbol "MEAN_ANOM" and its values from the symbol table.
C
...
C     The call
C
C     CALL SYDELC ( 'MEAN_ANOM', TABSYM, TABPTR, TABVAL )
C
C     deletes the symbol "MEAN_ANOM" from the symbol table. The
