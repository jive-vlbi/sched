C$Procedure     ZZEKAC08 ( EK, add class 8 column to segment )
 
      SUBROUTINE ZZEKAC08 (  HANDLE,  SEGDSC,  COLDSC,
     .                       DVALS,   NLFLGS,  WKINDX  )
 
C$ Abstract
C
C     Add an entire class 8 column to an EK segment.
C
C$ Disclaimer
C
...
C     DVALS      I   D.p. values to add to column.
...
C                    defined by DVALS is added to the specified segment
...
C     column.
C
C     Class 8 columns have fixed record counts and contain scalar,
...
C
C     This column had better be class 8.
C
      IF ( CLASS .NE. 8 ) THEN
     ...
     .                 'column #.  Class should be 8.'                 )
         CALL ERRINT ( '#', CLASS                                      )
         CALL ERRCH  ( '#', COLUMN                                     )
         CALL SIGERR ( 'SPICE(NOCLASS)'                                )
         CALL CHKOUT ( 'ZZEKAC08'                                      )
         RETURN
...
C
      ...
      CALL ZZEKACPS ( HANDLE, SEGDSC, DP, NPAGE, P, DATBAS )
 
C
C     We'll use FROM to indicate the element of DVALS we're
C     considering and TO to indicate the element of PAGE to write
C     to.
C
      TO  =  1
      CALL CLEARD ( PGSIZD, PAGE )
...
C
         ...
         IF (  ( TO .GT. DPSIZE ) .OR. ( FROM .EQ. NROWS )  ) THEN
C
...
C
      ...
      CALL DASUDI ( HANDLE, CMBASE+DBIX08, CMBASE+DBIX08, DATBAS )
...
C
         CALL ZZEKORDD ( DVALS,  NULLOK, NLFLGS, NROWS, WKINDX )
...
C        array.
C
         CALL DASUDI ( HANDLE, CMBASE+NFIX08, CMBASE+NFIX08, NFLBAS )
