C$Procedure      SYPSHC ( Push a value onto a particular symbol )
 
      SUBROUTINE SYPSHC ( NAME, VALUE, TABSYM, TABPTR, TABVAL )
 
C$ Abstract
C
C     Push a value onto a particular symbol in a character symbol table.
...
C     The call,
C
C     CALL SYPSHC ( 'PAULI', 'NEUTRINO', TABSYM, TABPTR, TABVAL )
C
...
C     The next call,
C
C     CALL SYPSHC ( 'MILLIKAN', 'PHOTOELECTRIC EFFECT',
...
C                        EXCLUSION PRINCIPLE
C
C     Note that a new symbol "MILLIKAN" was created by the last call.
C
C$ Restrictions
C
C     None.
...
C
      ELSE IF (  NVAL .GE. SIZEC ( TABVAL ) ) THEN
         ...
         CALL SETMSG ( 'SYPSHC: The addition of the value $ to the '  //
