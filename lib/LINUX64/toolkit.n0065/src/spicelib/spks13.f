C$Procedure SPKS13 ( S/P Kernel, subset, type 13 )
 
      SUBROUTINE SPKS13 ( HANDLE, BADDR, EADDR, BEGIN, END )
 
C$ Abstract
C
C     Extract a subset of the data in an SPK segment of type 13
...
C     routine SPKSUB.
C
C     It transfers a subset of a type 13 SPK data segment to
C     a properly initialized segment of a second SPK file.
C
C     The exact structure of a segment of data type 13 is described
C     in the section on type 13 in the SPK Required Reading.
C
C$ Examples
C
...
C
C     The type 9 subsetter knows how to do this job.
C
