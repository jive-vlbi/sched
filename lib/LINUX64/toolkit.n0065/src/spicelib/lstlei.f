C$Procedure   LSTLEI ( Last integer element less than or equal to )
...
C      LSTLEI     O   the index of the last element of ARRAY <= X.
C
C$ Detailed_Input
C
...
C      finds the index LSTLEI such that ARRAY(LSTLEI) is that number.
C
...
C      at most LOG_2(N) steps to find the value of LSTLEI.
C
...
C     that will insert duplicate items after existing items
...
C     then again LSTLEI (KEY, NKEYS, KEYS ) would be 3 and LOC would
C     be 4. After the calls to the routines INSLAI and INSLAD we
C     would have:
C
C           KEYS     VALUES     NKEYS = 7, NVALS = 7
C           ----     -------
...
C     existing entries. (LSTLTI is similar to LSTLEI except it finds
C     the last occurrance of an integer strictly less than a value.)
...
C
C
C     (Note: you should NOT use the
C     code outlined above as the basis of a sorting algorithm.
C     The NAIF routines SHELLI, SHELLD, SHELLC, ORDERI, ORDERD, ORDERC,
C     REORDI, REORDD and REORDC are much more efficient routines for
C     sorting arrays or sorting a set of parallel arrays using
C     one of the set as a key. The fragment presented here is useful
C     for performing update insertions into previously ordered arrays.)
C
...
C$ Index_Entries
C
C     last integer element less_than_or_equal_to
C
C-&
