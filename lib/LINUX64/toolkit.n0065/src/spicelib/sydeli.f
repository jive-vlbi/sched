C$Procedure      SYDELI ( Delete a symbol from a symbol table )
 
      SUBROUTINE SYDELI ( NAME, TABSYM, TABPTR, TABVAL )
 
C$ Abstract
C
C     Delete a symbol from an integer symbol table. The symbol
C     and its associated values are deleted.
C
C$ Disclaimer
C
...
C                NAME must match the symbol exactly.
C
C     TABSYM,
C     TABPTR,
...
C$ Examples
C
C     In the following example the subroutine SYDELI is used to delete
C     the symbol "pens" and its values from the symbol table.
C
...
C     The call
C
C     CALL SYDELI ( 'pens', TABSYM, TABPTR, TABVAL )
C
C     deletes the symbol "pens" from the symbol table. The
C     components of the symbol table on output are:
C
C        books   -->   5
C        erasers -->   6
C        pencils -->  12
