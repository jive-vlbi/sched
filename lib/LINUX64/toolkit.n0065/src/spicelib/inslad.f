C$Procedure      INSLAD (Insert at location in double precision array)
 
      SUBROUTINE INSLAD ( ELTS, NE, LOC, ARRAY, NA )
 
C$ Abstract
C
C      Insert one or more elements into a double precision array at
...
C      Let
C
C            ELTS(1) = 5.0D0        NA = 4      ARRAY(1) =  1.0D0
C            ELTS(2) = 6.0D0                    ARRAY(2) =  2.0D0
C            ELTS(3) = 7.0D0                    ARRAY(3) =  3.0D0
C                                               ARRAY(4) =  4.0D0
C
C      Then the call
C
C            CALL INSLAD ( ELTS, 3, 3, ARRAY, NA )
C
...
C
C
C      The following calls to INSLAD signal errors.
C
C      CALL INSLAD ( ELTS, 3, -1, ARRAY, NA )
C      CALL INSLAD ( ELTS, 3,  6, ARRAY, NA )
C      CALL INSLAD ( ELTS, 3,  2, ARRAY, -1 )
C      CALL INSLAD ( ELTS, 3, -1, ARRAY, -1 )
C
C$ Restrictions
C
...
C$ Index_Entries
C
C     insert at location in d.p. array
C
C-&
