C$Procedure     EKACED ( EK, add d.p. data to column )
 
      SUBROUTINE EKACED (  HANDLE,  SEGNO,  RECNO,  COLUMN,
     .                     NVALS,   DVALS,  ISNULL          )
 
C$ Abstract
C
C     Add data to an double precision column in a specified EK record.
C
C$ Disclaimer
C
...
C$ Examples
C
C     1)  Add the value 999.D0 to the third record of the column DCOL in
C         the fifth segment of an EK file designated by HANDLE.
C
C            CALL EKACED ( HANDLE, 5, 3, 'DCOL', 1, 999.D0, .FALSE. )
C
C
C     2)  Same as (1), but this time add a null value.  The argument
C         999.D0 is ignored because the null flag is set to .TRUE.
C
C            CALL EKACED ( HANDLE, 5, 3, 'DCOL', 1, 999.D0, .TRUE. )
C
C
C     3)  Add an array DBUFF of 10 values to the third record of the
C         column DARRAY in the fifth segment of an EK file designated by
C         HANDLE.
C
C            CALL EKACED ( HANDLE, 5, 3, 'DARRAY', 10, DBUFF, .FALSE. )
C
...
C$ Index_Entries
C
C     add double precision data to EK column
C     add data to EK
C     write double precision data to EK column
C
C-&
...
C
      ...
      IF (  ( DTYPE .NE. DP ) .AND. ( DTYPE .NE. TIME )  ) THEN
         ...
         CALL SETMSG ( 'Column # is of type #; EKACED only works '    //
     .                 'with d.p. or time columns.  RECNO = #; SEGNO '//
     .                 '= #; EK = #.'                                  )
         CALL ERRCH  ( '#',  COLUMN                                    )
         CALL ERRINT ( '#',  DTYPE                                     )
         CALL ERRINT ( '#',  RECNO                                     )
...
C
         CALL ZZEKAD02 ( HANDLE, SEGDSC, COLDSC, RECPTR, DVALS, ISNULL )
...
C
         CALL ZZEKAD05 ( HANDLE,  SEGDSC,  COLDSC,
