C$Procedure      SPKR13 ( Read SPK record from segment, type 13 )
 
      SUBROUTINE SPKR13 ( HANDLE, DESCR, ET, RECORD )
 
C$ Abstract
C
C     Read a single data record from a type 13 SPK segment.
C
C$ Disclaimer
C
...
C     RECORD     O   The desired type 13 SPK record.
C
C$ Detailed_Input
C
...
C
C     DESCR          is the descriptor for a type 13 SPK segment that 
...
C        for a properly constructed type 13 segment. No checks are
...
C$ Particulars
C
C     This subroutine will read a single record from a type 13 SPK
...
C     ET.
C
C     The exact format and structure of a type 13 SPK segment is
...
C$ Index_Entries
C
C     read record from type_13 spk segment
C
C-&
...
C
C     The type 9 reader knows how to obtain a type 13 record.
C 
