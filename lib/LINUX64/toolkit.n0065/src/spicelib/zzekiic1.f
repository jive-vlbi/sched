C$Procedure ZZEKIIC1 ( EK, insert into index, character, type 1 )
 
      SUBROUTINE ZZEKIIC1 ( HANDLE, SEGDSC, COLDSC, CKEY, RECPTR, NULL )
 
C$ Abstract
C
C     Insert into a type 1 EK index a record pointer associated with a
C     character key.  The key and record pointer determine the insertion
...
C
C     CKEY           is a character string key.
C
...
C     associated with a character, scalar column.  The type of the
C     index must be 1.
C
C     The ordinal position of the new item is determined by the key
C     CKEY.  The new item will follow the last item already present
C     in the column having a value less than or equal to CKEY.
C
...
C
C        Locate the predecessor of the input key, record pair.
C
         CALL ZZEKLERC (  HANDLE,  SEGDSC,  COLDSC,  CKEY,
     .                    RECPTR,  NULL,    PRVIDX,  PRVPTR  )
 
C
C        Insert the new record number right after the item we've found.
C
      ...
      ELSE
         ...
         CALL ZZEKCNAM ( HANDLE, SEGDSC, COLUMN )
