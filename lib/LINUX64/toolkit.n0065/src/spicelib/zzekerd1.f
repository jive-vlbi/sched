C$Procedure ZZEKERD1 ( EK, LLE using record pointers, d.p., type 1 )
 
      SUBROUTINE ZZEKERD1 (  HANDLE,  SEGDSC,  COLDSC,  DKEY,
     .                       RECPTR,  NULL,    PRVIDX,  PRVPTR  )
 
C$ Abstract
C
C     Find the last column value less than or equal to a specified key,
C     for a specifed d.p. EK column having a type 1 index, using
C     dictionary ordering on d.p. data values and record pointers.
C
C$ Disclaimer
C
...
C
C     DKEY,
C     RECPTR         are, respectively, a d.p. key and a pointer to
...
C         routine.
C
C     3)  If the tree is empty, PRVLOC and PRVPTR are set to zero.
...
C
C        Determine the order relation between DKEY and the column
...
C
     ...
     .                        DKEY,    0,       NULL           ) ) THEN
C
C           The column entry's value matches DKEY.  We must
