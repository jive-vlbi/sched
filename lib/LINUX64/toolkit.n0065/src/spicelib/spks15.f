C$Procedure SPKS15 ( S/P Kernel, subset, type 15 )
 
      SUBROUTINE SPKS15 ( HANDLE, BADDR, EADDR, BEGIN, END )
      IMPLICIT NONE
 
C$ Abstract
C
C     Extract a subset of the data in an SPK segment of type 15
...
C     routine SPKSUB. It transfers a subset of a type 15 SPK data
C     segment to a properly initialized segment of a second SPK file.
C
C     The exact structure of a segment of data type 15 is described
C     in the section on type 15 in the SPK Required Reading.
C
C$ Examples
C
...
C
C-    SPICELIB Version 1.0.0, 7-NOV-1994 (WLT)
C
C-&
 
C$ Index_Entries
C
C     subset type_15 spk segment
C
C-&
...
C     type 15 segment out of the file.
C
