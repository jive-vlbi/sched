...
C
      CALL LOCTIM ( TIMVEC, STATUS )
C
C     A status of zero (0) indicates success; anything else is a
C     failure.
C
      IF ( STATUS .NE. 0 ) THEN
         CALL SETMSG ( 'An error occurred while attempting to get'
     .   //            'the system time.. The string '             )
         CALL SIGERR ( 'SPICE(TIMEFETCHERROR)'                     )
         CALL CHKOUT ( 'ZZCPUTIM'                                    )
         RETURN
      END IF
C
C     Convert the date and time into double precision numbers for output
C     and leave.
C
      TVEC(1) = DBLE( TIMVEC(1) )
      TVEC(2) = DBLE( TIMVEC(2) )
      TVEC(3) = DBLE( TIMVEC(3) )
      TVEC(4) = DBLE( TIMVEC(4) )
      TVEC(5) = DBLE( TIMVEC(5) )
      TVEC(6) = DBLE( TIMVEC(6) )
