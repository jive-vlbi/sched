C$Procedure      SYPOPI ( Pop a value from a particular symbol )
 
      SUBROUTINE SYPOPI ( NAME, TABSYM, TABPTR, TABVAL, VALUE, FOUND )
 
C$ Abstract
C
C     Pop a value associated with a particular symbol in an integer
...
C     The call,
C
C     CALL SYPOPI ( 'pens', TABSYM, TABPTR, TABVAL, VALUE, FOUND )
C
...
C                     24
C
C     FOUND is TRUE, and VALUE is 10.
C
C
C     The next call,
C
C     CALL SYPOPI ( 'erasers', TABSYM, TABPTR, TABVAL, VALUE, FOUND )
C
C     modifies the contents of the symbol table to be:
C
C        books   -->   5
...
C                     24
C
C      FOUND is TRUE, and VALUE is 6. Note that because
C      "erasers" had only one value associated with it, it was removed
...
C$ Version
C
C-     SPICELIB Version 1.0.2, 10-DEC-2002 (LSE)
C
C         Fixed typo (FISSION') in header
C
...
C
         ...
         CALL REMLAI ( 1, LOCVAL, TABVAL(1), NVAL )
