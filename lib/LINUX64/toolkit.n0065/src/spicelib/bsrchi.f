C$Procedure            BSRCHI ( Binary search for an integer value )
...
C      assumed to be in increasing order. Return the index of the
...
C      BSRCHI     O   Index of VALUE in ARRAY. (Zero if not found.)
C
C$ Detailed_Input
C
...
C$ Detailed_Output
C
C      BSRCHI      is the index of the input value in the input array.
C                  If ARRAY does not contain VALUE, BSRCHI is zero.
C
C                  If ARRAY contains more than one occurrence of VALUE,
C                  BSRCHI may point to any of the occurrences.
C
C$ Parameters
C
C     None.
...
C      Then
C
C            BSRCHI ( -11, 4, ARRAY )    = 1
C            BSRCHI (  22, 4, ARRAY )    = 3
C            BSRCHI ( 751, 4, ARRAY )    = 0
C
C$ Restrictions
C
C      ARRAY is assumed to be sorted in increasing order. If this
C      condition is not met, the results of BSRCHI are unpredictable.
C
C$ Exceptions
C
C      Error free.
...
C$ Index_Entries
C
C     binary search for an integer value
C
C-&
