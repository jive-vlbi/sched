C$Procedure      SYPUTC ( Set the values associated with a symbol )
 
      SUBROUTINE SYPUTC ( NAME, VALUES, N, TABSYM, TABPTR, TABVAL )
 
C$ Abstract
C
C     Set the values of a particular symbol in a character symbol table.
...
C     the call
C
C     CALL SYPUTC ( 'EINSTEIN', VALUES, 3, TABSYM, TABPTR, TABVAL )
C
C     modifies the contents of the symbol table to be:
C
C        BOHR      -->   HYDROGEN ATOM
C        ENISTEIN  -->   SPECIAL RELATIVITY
C                        PHOTOELECTRIC EFFECT
C                        BROWNIAN MOTION
C        FERMI     -->   NUCLEAR FISSION
C        PAULI     -->   NEUTRINO
...
C     The call,
C
C     CALL SYPUTC ( 'PAULI', VALUES, 3, TABSYM, TABPTR, TABVAL )
C
C     modifies the contents of the symbol table to be:
C
C        BOHR      -->   HYDROGEN ATOM
...
C                        BROWNIAN MOTION
C        FERMI     -->   NUCLEAR FISSION
C        PAULI     -->   SPECIAL RELATIVITY
C                        PHOTOELECTRIC EFFECT
C                        BROWNIAN MOTION
C
C     Note that the previous values associated with PAULI have been
...
C
      IF ( N .LT. 1 ) THEN
         CALL SETMSG ( 'SYPUTC: The dimension of the values array is' //
     .                 'less than one.' )
...
C        the value table.
C
         CALL INSLAC ( VALUES, N, LOCVAL, TABVAL(1), NVAL )
         CALL SCARDC ( NVAL, TABVAL )
