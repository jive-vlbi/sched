C$Procedure      ZZEKDE03 ( EK, delete column entry, class 3 )
 
      SUBROUTINE ZZEKDE03 ( HANDLE, SEGDSC, COLDSC, RECPTR )
 
C$ Abstract
C
C     Delete a specified class 3 column entry from an EK record.
C
C$ Disclaimer
C
...
C
C        Get the character count for the entry.
C
         CALL ZZEKGEI ( HANDLE, DATPTR, NCHARS )
