C$Procedure    ZZEKILLE ( EK, indirect, last elt less than or equal to )
 
      INTEGER FUNCTION ZZEKILLE (  HANDLE,  SEGDSC,  COLDSC,  NROWS,
...
C     than or equal to a specified value, where the order relation is
C     given by an order vector in a specified DAS file.
C
C$ Disclaimer
C
...
C     that points to an array element that is less than or equal to
C     the input value of the same data type as the column.
C
C$ Detailed_Input
C
...
C
C     The function returns the index of the last order vector element
C     that points to a column entry that is less than or equal to
C     whichever of CVAL, DVAL, or IVAL has the same data type as the
C     input column.  If the least element of the column is greater than
C     the input value of the matching type, the function returns the
C     value zero.
C
C$ Parameters
C
...
C         called by this routine.
C
C     4)  If any of SEGBAS, COLDSC, or NROWS are invalid, this routine
...
C
         ...
         CALL CHKOUT ( 'ZZEKILLE'                                )
         RETURN
      END IF
 
C
C     Hand off the problem to the LLE routine of the correct type.
C
      COLTYP  =  COLDSC ( TYPIDX )
 
      IF ( COLTYP .EQ. CHR ) THEN
 
         CALL ZZEKLLEC ( HANDLE, SEGDSC, COLDSC, CVAL, ZZEKILLE, REC )
      ...
      ELSE IF ( COLTYP .EQ. DP ) THEN
         ...
         CALL ZZEKLLED ( HANDLE, SEGDSC, COLDSC, DNUM, ZZEKILLE, REC )
 
 
      ELSE IF ( COLTYP .EQ. TIME ) THEN
 
         CALL ZZEKLLED ( HANDLE, SEGDSC, COLDSC, DVAL, ZZEKILLE, REC )
      ...
      ELSE IF ( COLTYP .EQ. INT ) THEN
         ...
         CALL ZZEKLLEI ( HANDLE, SEGDSC, COLDSC, INUM, ZZEKILLE, REC )
