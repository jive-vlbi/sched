C$Procedure      SYSETI ( Set the value associated with a symbol )
 
      SUBROUTINE SYSETI ( NAME, VALUE, TABSYM, TABPTR, TABVAL )
 
C$ Abstract
C
C     Set the value of a particular symbol in an integer symbol table.
...
C     The call,
C
C     CALL SYSETI ( 'pens', 36, TABSYM, TABPTR, TABVAL )
C
...
C     "pens" have been deleted, and now only the new value is
C     associated with the symbol.
C
C
C     The next call,
C
C     CALL SYSETI ( 'desks', 31, TABSYM, TABPTR, TABVAL )
C
C     modifies the contents of the symbol table to be:
C
C        books   -->   5
...
C        pens    -->  36
C
C     Note that the new symbol "desks" was created by the last call.
...
C
      IF ( OLDSYM ) THEN
         ...
         IF ( DIMVAL .GT. 1 ) THEN
            CALL REMLAI ( DIMVAL-1, LOCVAL, TABVAL(1), NVAL )
...
C
      ELSE IF ( NSYM .GE. SIZEC ( TABSYM ) ) THEN
            ...
            CALL SETMSG ( 'SYSETI: Addition of the new symbol # '     //
