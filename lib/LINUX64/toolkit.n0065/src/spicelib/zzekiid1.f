C$Procedure ZZEKIID1 ( EK, insert into index, d.p., type 1 )
 
      SUBROUTINE ZZEKIID1 ( HANDLE, SEGDSC, COLDSC, DKEY, RECPTR, NULL )
 
C$ Abstract
C
C     Insert into a type 1 EK index a record pointer associated with a
C     d.p. key.  The key and record pointer determine the insertion
...
C
C     DKEY           is a double precision key.
C
...
C
C     2)  If the data type of the input column is not double precision,
...
C     associated with a double precision, scalar column.  The type of
C     the double precision index must be 1.
C
C     The ordinal position of the new item is determined by the key
C     DKEY.  The new item will follow the last item already present
C     in the column having a value less than or equal to DKEY.
C
...
C
         CALL ZZEKLERD (  HANDLE,  SEGDSC,  COLDSC,  DKEY,
