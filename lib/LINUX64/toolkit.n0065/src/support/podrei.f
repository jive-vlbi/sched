C$Procedure      PODREI ( Pod, remove elements, integer )
 
      SUBROUTINE PODREI ( N, LOC, POD )
      IMPLICIT NONE
...
C     by hand,
C
C        CALL PODONI ( POD, OFFSET, NUMBER )
C        END = OFFSET + NUMBER
C
C        CALL REMLAI ( N,   OFFSET + LOC, POD(1), END )
C        CALL SCARDI ( END,               POD         )
C
...
C
      ...
      CALL REMLAI ( N,   OFFSET + LOC, POD(1), END )
      CALL SCARDI ( END,               POD         )
