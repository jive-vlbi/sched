C$Procedure      NTHUQT ( N'th unquoted token )
 
      SUBROUTINE NTHUQT ( STRING, N, EQUOTE, WORD, LOC )
 
C$ Abstract
C
C    This routine finds the N'th non-quoted token in a string.
...
C      N          I   Index of the token to be returned.
C      EQUOTE     I   An extra quote character.
C      WORD       O   The N'TH unquoted token in STRING.
C      LOC        O   Location of WORD in STRING.
C
C$ Detailed_Input
C
C      STRING     is the input string to be parsed. It contains
C                 some number of token, where a token is any string
C                 of consecutive non-blank, non-comma characters
C                 not between balanced quotes.
C
C      N          is the index of the token to be returned. (One for
C                 the first token, two for the second, and so on.)
C
C      EQUOTE     is a special character that users may supply so
C                 that specially marked strings will be skipped
C                 in the selection of tokens.  If you do not want
C                 any specially marked strings use a blank for EQUOTE
C
C$ Detailed_Output
C
C      WORD       is the N'th token in STRING. If STRING is blank,
...
C$ Particulars
C
C      NTHUQT, like NTHWD, is useful primarily for parsing input
C      commands consisting of one or more tokens, where a token is
C      defined to be any sequence of consecutive non-blank characters.
C
C      The chief difference between the two routines is that
C      NTHUQT treats all character starting at and through
...
C$ Index_Entries
C
C     Find the n'th unquoted token in a string
C
C-&
C
C     Spice Functions
