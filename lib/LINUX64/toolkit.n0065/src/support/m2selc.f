C$Procedure M2SELC ( META/2 --- select a named word )
 
      SUBROUTINE M2SELC ( NAME, STRING, NTH, FOUND, WORD )
      IMPLICIT NONE
...
C
C     WORD       is the word in STRING that corresponds to the request
C                specified by NAME and NTH.
C
C$ Parameters
C
C     None.
...
C
C     2) If the portion of STRING extracted is NOT a word, the error
...
C$ Index_Entry
C
C     Extract n'th word matching a named template word
C
C-&
