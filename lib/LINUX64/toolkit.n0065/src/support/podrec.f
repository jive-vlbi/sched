C$Procedure      PODREC ( Pod, remove elements, character )
 
      SUBROUTINE PODREC ( N, LOC, POD )
      IMPLICIT NONE
...
C        END = OFFSET + NUMBER
C
C        CALL REMLAC ( N,   OFFSET + LOC, POD(1), END )
C        CALL SCARDC ( END,               POD         )
C
