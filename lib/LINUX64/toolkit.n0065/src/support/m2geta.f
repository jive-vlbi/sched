C$Procedure M2GETA ( META/2 --- get all of a named word )
 
      SUBROUTINE M2GETA ( NAME, STRING, FOUND, WORD )
      IMPLICIT NONE
 
C$ Abstract
C
C     Get all substrings associated with a matched, named META/2
C     template word and put it into the specified WORD.
C
C$ Disclaimer
C
...
C     WORD       O   the matching word extracted from STRING.
C
C$ Detailed_Input
C
...
C
C     WORD       is the full substring in STRING that corresponds to
C                the request specified by NAME.
C
C$ Parameters
C
C     None.
...
C$ Exceptions
C
C     1) If WORD is not sufficiently large to hold all of the characters
C        the error 'META/2(INSUFFICIENTSPACE)' will be signalled.
C
C     2) If the portion of STRING extracted does not begin and end
C        with a word, the error 'META/2(CORRUPTEDINPUTSTRING)' will
C        be signalled.
C
C$ Files
C
...
C$ Index_Entry
C
C     Extract all words matching a named template word
C
C-&
...
C
C     First look up the beginning and endings of the requested
C     substring.
C
...
C
C     First find out how many substrings are associated with this name.
C
      LAST = M2HAVE ( NAME )
...
C
C     Now get the beginning and ending of all the stuff associated
C     with this word.
C
      CALL M2VGET ( NAME, 1,    FOUND, B(1), E(1) )
      CALL M2VGET ( NAME, LAST, FOUND, B(2), E(2) )
 
C
C     First make sure there are no obvious pathologies about the string
      ...
      DO I = 1, 2
         ...
         IF ( P .GT. 0 ) THEN
            IF ( STRING(P:P) .NE. ' ' ) THEN
               CALL CHKIN  ( 'M2GETA' )
               CALL SETMSG ( 'The input string has been modified '    //
     .                       'since it passed syntax validation in '  //
     .                       'META/2. '                               )
         ...
         IF ( F .LT. L ) THEN
            IF ( STRING(F:F) .NE. ' ' ) THEN
               CALL CHKIN  ( 'M2GETA' )
               CALL SETMSG ( 'The input string has been modified '    //
     .                       'since it passed syntax validation '     //
     .                       'in META/2. '                            )
               CALL SIGERR ( 'META/2(CORRUPTEDINPUTSTRING)'           )
               CALL CHKOUT ( 'M2GETA' )
               RETURN
         ...
         IF (      ( STRING(B(I):B(I)) .EQ. ' ' )
     .        .OR. ( STRING(E(I):E(I)) .EQ. ' ' ) ) THEN
            CALL CHKIN  ( 'M2GETA' )
...
C
C     Next make sure there is room to hold everything.
C
      W = LEN(WORD)
 
      IF ( W .LT. E(2) - B(1) + 1 ) THEN
         CALL CHKIN  ( 'M2GETA' )
         CALL SETMSG ( 'There is not sufficient space in the output ' //
     .                 'string to hold the requested word. '          )
         CALL SIGERR ( 'META/2(INSUFFICIENTSPACE)' )
         CALL CHKOUT ( 'M2GETA' )
         RETURN
      END IF
