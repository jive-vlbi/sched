...
struct TkPgplot {
  ...
  int fixsize;              /* Don't resize the pixmap to match the size */
                            /*  of the window when pgpage() is called. */
                      /* Private attributes */
...
static Tk_ConfigSpec configSpecs[] = {
  ...
  {TK_CONFIG_BOOLEAN,
     "-fixsize", "fixsize", "FixSize",
     0, Tk_Offset(TkPgplot, fixsize), 0}
,
...
typedef enum {
  ...
  TKPG_HLINE_CURSOR = 5, /* Horizontal line cursor at y=ref.y */
  TKPG_VLINE_CURSOR = 6, /* Vertical line cursor at x=ref.x */
...
/*
 * Specify whether the size of the underlying pixmap should be fixed,
 * or vary to match the window size.
 */
  pgx_fix_size(pgx, tkpg->fixsize);
/*
 * Refresh the window.
 */
 ...
 */
  return w && Tk_IsTopLevel(w) ? w : main_w;
}
