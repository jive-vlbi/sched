      SUBROUTINE CHK4REC( KS, ERRS )
C
C     Routine for SCHED called by CHKSET that checks items related
C     to Mark IV RECORDERS's.  
C
C     Note that VLBA track numbering schemes are assumed. 
C     This may need to be cleaned up.
C
      INCLUDE  'sched.inc'
      INCLUDE  'schset.inc'
C
      ...
      LOGICAL       ERRS
C ----------------------------------------------------------------------
      IF( DEBUG ) CALL WLOG( 1, 'CHK4REC: starting.' )
C
...
C
     ...
     5    FORMAT(KS) .NE. 'MARKIII' ) THEN
         MSGTXT = ' '
         WRITE( MSGTXT, '( A, A)' )
     1       'CHK4REC: Invalid FORMAT for Mark IV recording '//
...
C
     ...
     1             'CHK4REC: Requested tracks ', TRK1, TRK2, 
...
C
      DO ITRK = 2, 33
         IF( TRKUSED(ITRK) .GT. 1.0 ) THEN
            CALL WLOG( 1, 'CHK4REC: The setup file requests that '//
     1        'a track be overwritten.' )
            CALL WLOG( 1, 'CHK4REC: Check assignments and room for '//
     ...
     1       TRKUSED(ITRK) .LT. 1.0 ) THEN
            CALL WLOG( 1, 'CHK4REC: Insufficient data routed to a'
...
C
     ...
     1    TAPEMODE(KS) .NE. 4 .AND. TAPEMODE(KS) .NE. 8 ) THEN
         CALL WLOG( 1, 'CHK4REC: tpmode must be 1, 2, 4, or 8' )
         ERRS = .TRUE.
      END IF
CC
C      CR (25 Oct 2004): barrel-roll now possible (but not for JIVE, see
C                        chkjive.f)
CC     Barrel-rolling currently not enabled for MkIV, waits firmware upgrade
CC
...
C      END IF
      IF( FANOUT(KS) .LT. 0.9 ) THEN
         CALL WLOG( 1, 'CHK4REC: Fan-in currently not supported MkIV')
         ERRS = .TRUE.
      END IF
C
...
C
      IF( TSTA .EQ. 0 ) THEN
         WRITE(*,*) 'CHK4REC: Could not relate setup station and ',
...
C     1       ' 135ips (L) or 80ips (H) not often exercised... '
     ...
     2          'for the JIVE correlator. Testers *only* may specify ',
     3          'MODETEST'
            CALL ERRLOG ( MSGTXT )
         ENDIF
