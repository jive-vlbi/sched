      SUBROUTINE WRAPZONE2( IUNIT, ISCN, ISTA, ZONE )
C
...
C
      ...
      CHARACTER     ZONE*(*), ZNAMES(6)*5, ZNAMES2(6)*5
C
      INTEGER       KSTA, I
C -------------------------------------------------------------------
      KSTA = STANUM(ISTA)
...
C
     ...
     1            // 'F6.2, A, A, A, I2 )' ) '     pointing_sector = ', 
     2            ZNAMES(I),
     3            ' : az :', ZLOWAZ(I), ' deg:', ZHIGHAZ(I), 
     4            ' deg: el :', ZLOWEL(I), ' deg:', ZHIGHEL(I), 
     5            ' deg: ', ZNAMES2(I), '; * cable wrap zone', I
...
C
         IF( ISCN .EQ. 0 ) THEN
            MSGTXT = ' '
            WRITE( MSGTXT, '( A, A )' )
     1         'WRAPZONE2: Station ', STATION(KSTA), 
