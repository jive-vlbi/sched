...
C
         IF( ISTA .LT. 0 ) 
     1       CALL ERRLOG(' VXWRDA2: no station for $DAS def ')
C
...
C
     ...
     2          'equip = rack', COL,
     3          DBE(KS)(1:LEN1(DBE(KS))), COL, LNK, DAR(ISCAT), SEP
...
C
     ...
     1            1X, A1, 1X, A1, A, 1X, A1, 1X )' ) 
     2          'equip = rack', COL,
     3          DBE(KS)(1:LEN1(DBE(KS))), COL, LNK, 'Mark4', SEP
C            WRITE( IVEX, '( 5X, A, A, A1)' )
C     1          'electronics_rack_type = ',
C     2          'Mark4', SEP
         ELSE
            CALL ERRLOG(' VXWRDA2: Unknown DAR of type: '//
     1          DAR(ISCAT) )
         END IF
C
...
C
     ...
     2           DISK(STANUM(ISTA)) .EQ. 'MARK5C'    ) THEN
                ...
                WRITE( IVEX, '( 5X, A, 1X, A1, 1X, A, 
     1           1X, A1, 1X, A1, A, A1  )' )
     2              'equip = recorder', COL,
     3              WRDISK(1:LEN1(WRDISK)), COL, LNK, 
     4              DISK(STANUM(ISTA)), SEP
     ...
     4              DISK(STANUM(ISTA)), SEP
             ELSE IF( DISK(STANUM(ISTA)) .EQ. 'LBADR' ) THEN
                WRITE( IVEX, '( 5X, A, 1X, A1, 1X, A, A1 )' )
     1              'equip = recorder', COL,
     2              'S2', SEP
C     2              'Mark5A', SEP
             ELSE
                CALL ERRLOG(' VXWRDA2: Unknown recorder of type: '//
     1              RECORDER(ISCAT) )
             END IF
         END IF
C
...
C
C         WRITE( IVEX, '( 5X, A, I1, A1 )' ) 'number_drives = ',
C     1       STNDRIV(STANUM(ISTA)), SEP
C
...
C
     ...
     1          'VXWRDA2: WARNING: More than 2 heads on ',
...
C           WRITE (HEAD, '( A )' ) ''
     ...
     3       NHEADS(STANUM(ISTA)) .GT. 9 ) THEN
            CALL ERRLOG('VXWRDA2 inconsitent number of'//
     1          ' drives/headstacks')
C         ELSE
C            DO IDRIV = 1, STNDRIV(STANUM(ISTA)) 
C               DO JHEAD = 1, NHEADS(STANUM(ISTA))
C                  WRITE( IVEX, '( 5X, A, I1, 1X, A1, 1X, A, 
C     1                1X, A1, 1X, I1, 1X, A1 )' ) 
C     2                'headstack = ', 
C     3                JHEAD + NHEADS(STANUM(ISTA))*(IDRIV-1), COL,
C     4                HEAD, COL, IDRIV-1, SEP
C               END DO
C            END DO
         END IF
C
...
C        this gets fixed
C
C         IF( USETAPE(ISTA) .OR. .NOT. VEXTEST ) THEN
C            WRITE( IVEX, '( 5X, A, A, A1, I2, A, A1, I2, A, A1, 
C     1          I3, A, A1 )' ) 'tape_motion = ', 
C     2          'adaptive ', COL, 0, ' min', COL,
C     3          0, ' min', COL, 10, ' sec', SEP
C         END IF
C
...
C
      ...
      WRITE( IVEX, '( A )' ) COMLIN
      IF( VIOLFS )
     1    CALL WLOG( 1,'VXWRDA2: More than 2 headstacks '//
