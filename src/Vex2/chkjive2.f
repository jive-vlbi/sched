...
C ---------------------------------------------------------------------
      IF( DEBUG ) CALL WLOG( 1, 'CHKJIVE2: starting.' )
C
...
C
         IF( ROLLERR .NE. 0 ) THEN
            CALL WLOG( 1, 'CHKJIVE2:  JIVE Correlator does '//
     1           'not do barrel rolling. Please set barrel="roll_off"' )
            CALL WRTMSG( 0, 'CHKJIVE2', 'barrelroll')
            CALL ERRSET( ROLLERR )
         END IF
C
...
C
         IF( SPUPERR .NE. 0 ) THEN
            CALL WLOG( 1, 'CHKJIVE2:  JIVE Correlator does not '//
     1          'currently do speed-up correlation (Oct 2004)' )
            CALL WLOG( 1, '         or testers set MODETEST ')
            CALL WRTMSG( 0, 'CHKJIVE2', 'slowdatarate')
            CALL ERRSET( SPUPERR )
         END IF
C
