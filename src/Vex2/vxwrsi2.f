...
C
         IF( ISTA .LT. 0 ) 
     1        CALL ERRLOG(' VXWRSI2: no station for $SITE def ')
C
         ISCAT = STANUM(ISTA)
C
         WRITE( IVEX, '( A1 )' ) COM
...
C
     ...
     1             'VXWRSI2: WARNING: More than 60 points on ',
...
C
      IF( VIOLFS )
     1    CALL WLOG( 1,'VXWRSI2: More than 60 points on horizon '//
