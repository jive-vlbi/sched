...
C
      IF( NMDVEX .GT. 20 ) THEN
         MSGTXT = ' ' 
         WRITE( MSGTXT, '( A, I3, A )' ) 
     1       '++++ VXWRMD2: WARNING: More than 20 VEX modes (', NMDVEX,
     2       ') in this schedule. '
         CALL WLOG( 1,MSGTXT)
         MSGTXT = ' ' 
         WRITE( MSGTXT, '( 2A )' ) 
...
C
                     IF( LPOS .GT. MAXLINE - 8 ) THEN
                        CALL ERRLOG( 'VXWRMD2: Too many stations '//
...
C
     ...
     3                ' <= obsolete definition'
         ...
         END DO
C
C        write the Datastream section for this mode
C
         DO I = 1, NMODTR(IMODE)
            IXX = IMODTR(I,IMODE)
C
...
C
            IF( NMODTR(IMODE) .EQ. -999 ) THEN
               IF ( FORMAT(1)(1:6) .EQ. 'MARK5B') THEN
                   WRITE( LINE, '( 5X, A, A, A1, A, A1 )' ) 
     1             'ref $BITSTREAMS = ',
...
C
     ...
     3                ' <= obsolete definition'
         ...
         END DO
C
C        write the EXTENSIONS links for this mode (same as IF)
C
         DO I = 1, NMODIF(IMODE)
            IXX = IMODIF(I,IMODE)
C
