...
C ---------------------------------------------------------------------
      IF( SDEBUG ) CALL WLOG( 1, 'CHKV4DAR2: Starting.' )
C
C
C
     ...
     7       FORMAT(KS) .NE. 'S2' ) THEN
            MSGTXT = ' '
            WRITE( MSGTXT, '( A, A)' )
     1          'CHKV4DAR2: Invalid FORMAT for wideband observing '//
...
C
         IF( .NOT. OK ) THEN
            ...
            SETMSG = ' '
            WRITE( SETMSG, '( 3A, I4 )' )
     1          'CHKV4DAR2: Illegal IF input ', IFCHAN(ICH,KS),
