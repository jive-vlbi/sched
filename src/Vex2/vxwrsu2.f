...
C
                  ...
                  IF ( NVXSRC .GT. 1000 ) THEN
                     WRITE( MSGTXT, '( A )' ) 
     1                   'VXWRSU2: WARNING: More than 1000 sources'// 
...
C
            IF( .NOT. FOUND ) THEN
               MSGTXT = ' '
               WRITE( MSGTXT, '( A, A )' )
     1            'VXWRSU2: A catalog source is marked as used but ',
     2                  'the alias used is not identified.'
               CALL WLOG( 1, MSGTXT )
               MSGTXT = ' '
               WRITE( MSGTXT, '( A, I5, 2A )' )
     1            'VXWRSU2: Source number is: ', ISRC, 
     2            '  The first alias is: ', 
     3            SOURCE(1,ISRC)
               CALL WLOG( 1, MSGTXT )
C                
               CALL ERRLOG('VXWRSU2: Probable program bug.'//
     1             '  Please report.' )
            END IF
         END IF
      END DO
