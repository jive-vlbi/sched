      SUBROUTINE CHK4DAR2( KS, SNBBC, ERRS )
C
...
C
      ...
      REAL           LOWEDGE, HIEDGE
C ----------------------------------------------------------------------
      IF( DEBUG ) CALL WLOG( 1, 'CHK4DAR2: starting.' )
C
C
C
...
C
     ...
     7       FORMAT(KS) .NE. 'S2' ) THEN
            MSGTXT = ' '
            WRITE( MSGTXT, '( A, A)' )
     1          'CHK4DAR2: Invalid FORMAT for wideband observing '//
...
C
      DO I = 1, 4
         IF( IFDIST(I,KS) .NE. '0'  ) THEN
            MSGTXT = 'CHK4DAR2: IFDIST specified: '//IFDIST(I,KS)// 
...
C
     ...
     3       SAMPRATE(KS) .NE. 16.  .AND. SAMPRATE(KS) .NE. 32. ) THEN
            MSGTXT = ' '
            WRITE( MSGTXT, '( A, F8.3 )' )
     1          'CHK4DAR2: Invalid SAMPRATE specified: ', SAMPRATE(KS)
            CALL WLOG ( 1, MSGTXT )
            ERRS = .TRUE.
         END IF
C
         IF( BITS(1,KS) .NE. 1 .AND. BITS(1,KS) .NE. 2 ) THEN
            MSGTXT = ' '
            WRITE( MSGTXT, '( A, I5 )' ) 'CHK4DAR2: BITS must be 1 or 2'
...
C
     ...
     5       IFCHAN(ICH,KS) .NE. '3O' ) THEN
            CALL WLOG ( 1, 'CHK4DAR2: IFCHAN ''' // IFCHAN(ICH,KS) //
     1          ''' not 1N,1A,2A,2N,3N or 3O' )
            ERRS = .TRUE.
         END IF
         IF( IFCHAN(ICH,KS)(1:1) .EQ. '3' ) THEN
                CALL WLOG ( 1, 'CHK4DAR2: Not many astronomical ' //
...
C
     ...
     3             IFCHAN(JCH,KS) .EQ. '2A' )) THEN
                  IF( .NOT. ALDONE ) THEN 
                     MSGTXT = ' '
                     WRITE( MSGTXT, '( A  )' )
     1                   'CHK4DAR2: IFCHAN setup assigns IFbox '//
...
C
         IF( BBC(ICH,KS) .LT. 1 ) THEN
            CALL WLOG( 1, 'CHK4DAR2: BBC number less than 1! ')
            ERRS = .TRUE.
         END IF
         IF( BBC(ICH,KS) .GT. SNBBC ) THEN
            MSGTXT = ' '
            WRITE( MSGTXT, '( A, I3, A, I3, 2A )' )
     1         'CHK4DAR2: Channel', ICH, ' uses a BBC (', BBC(ICH,KS),
...
C           Check for consistency with standard geodetic patching
     ...
     1              'CHK4DAR2: MkIV geodetic patching '//
...
C           This means we have astro patching
     ...
     1             .AND. IFCHAN(ICH,KS) .NE. '2N' ) THEN
                  MSGTXT = ' '
                  WRITE( MSGTXT, '( A, I2, A, A3, A, A )' )
     1                'CHK4DAR2: MkIV astronomical patching '//
...
C
     ...
     1       BBSYN(ICH,KS) .GT. 500.0 ) THEN
            MSGTXT = ' '
            WRITE( MSGTXT, '( A, F8.2 )' )
     1         'CHK4DAR2: BBSYN not between 100 and 500: ', 
     2         BBSYN(ICH,KS)
            CALL WLOG( 1, MSGTXT )
...
C
     ...
     1           'CHK4DAR2: Improper patching BBC#',
     2            BBC(ICH,KS), 
     3            ' is not on Low.'
                  CALL WLOG( 1, MSGTXT )
               GEOSPLPA = .TRUE.
...
C
               MSGTXT = ' '
               WRITE( MSGTXT, '( A, I2.2, A )' )
     1             'CHK4DAR2: Inconsistent patching BBC#',
     2             BBC(ICH,KS), 
     3             ' is not on High.'
               CALL WLOG( 1, MSGTXT )
               GEOSPLPA = .TRUE.
...
C
     ...
     4       .AND. BBFILT(ICH,KS).NE.16. ) THEN
            MSGTXT = ' '
            WRITE( MSGTXT, '( A, F8.2 )' )
     1          'CHK4DAR2: Invalid BBFILTER specified: ',
     2          BBFILT(ICH,KS)
            CALL WLOG( 1, MSGTXT )
            ERRS = .TRUE.
C
...
C
     ...
     1             'CHK4DAR2: ', SETNAME(KS)(1:LEN1(SETNAME(KS))),
...
C
     ...
     1       BBSYN2(ICH,KS) .GT. 500.0 ) ) THEN
            MSGTXT = ' '
            WRITE( MSGTXT, '( A, F8.2 )' )
     1         'CHK4DAR2: BBSYN2 not between 100 and 500: ', 
     2          BBSYN2(ICH,KS)
            CALL WLOG( 1, MSGTXT )
            ERRS = .TRUE.
         END IF
...
C
         IF( BBC(ICH,KS) .GT. 8 .AND. FANOUT(KS) .EQ. 4.0 ) THEN 
            WRITE( MSGTXT, '( A, A )' ) 
     1          'CHK4DAR2: BBC9 and above cannot be used in 1:4',
...
C
      IF( PATERR ) THEN
        CALL WRTMSG(0, 'CHK4DAR2', 'astropatch' )
      END IF
C
      IF( GEOPATER .OR. GEOSPLPA ) THEN
        CALL WRTMSG(0, 'CHK4DAR2', 'geopatch' )
      END IF
C
      IF( M4PATCH(KS) .EQ. 'FREE' ) THEN
        CALL WRTMSG(0, 'CHK4DAR2', 'freepatch' )
      ELSE IF( M4PATCH(KS) .EQ. 'GEO1' ) THEN
         WRITE( MSGTXT, '(A)' )
...
C
      IF( .NOT. USENOR ) THEN
         WRITE(MSGTXT, '( A, A, A)' )
     1       'CHk4DAR2: Neither IFCHAN 1N nor 2N have been assigned ',
...
C
     ...
     1       'CHk4DAR2: Requested mixed patching for ',SETSTA(1,KS),
     2       ' in setup ',SETNAME(KS)(1:LEN1(SETNAME(KS)) )
         CALL WLOG( 1, MSGTXT )
         WRITE(MSGTXT, '( A, A )' )
...
C
      IF (GEOSPLPA) THEN
         WRITE(MSGTXT, '( A, A, A, A)' )
     1       'CHk4DA2: Requested non-standard geodetic patching for ',
