...
C
      ...
      DO ISTA = 1, NSTA
         ...
         END IF
      END DO
      IF( ISET .LT. 0 ) CALL ERRLOG('VXTRAMD2: Scan not observed')
  100 CONTINUE
C     
...
C
     ...
     1             NINT( 1E3 * BW(1,ISCN)), 'kHz'
      ...
      END IF
      MDLINK(NMDVEX) = NAME
      CALL VXUNQL2( NMDVEX, MDLINK )
C
      IF( DEBUG ) THEN 
         WRITE( MSGTXT, '( A, A, A )' )
     1       'VXSCNS2 (VXTRAMD2): Mode change in schedule',
...
C     Oct 16, 2011.
C 
      CALL VXTRAFQ2(ISCN, NMDVEX, IMODE, IFQ)
C     
...
C
      ...
      ELSE IF( UPCAL .EQ. '5MHZ' ) THEN
         PHTONE = 5.0
      ELSE 
         CALL ERRLOG( 'VXSCNS2: Invalid PCAL: '
     1       //FSPCAL(IFS)//
     1       ' - Must be 1MHz, 5MHz, or off.' )
      END IF   
      CALL VXTON2_2( -1, ISET, NVXCHN(NFQVEX), 
     1    VXLOSUM(1,IFQ), 
...
C
         CALL VXTRAPH2( ISCN, NMDVEX, IMODE, PHTONE,
     1       NTMP, NTONTMP, ITONTMP )
C     
C        done inventing new PH 
C        do same for IF section
C     
         CALL VXTRAIF2( ISCN, NMDVEX, IMODE, PHTONE )
