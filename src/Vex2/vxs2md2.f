      SUBROUTINE VXS2MD2( ISET, S2MODE, VALID, NS2USD, IS2USD, DOWARN )
C 
...
C
     ...
     1    THEDAR .EQ. 'VLBAG' ) THEN
         IF( DEBUG ) CALL WLOG( 1,'       Using VXS2VL2 ')
         CALL VXS2VL2( NCHAN(ISET), BBC(1, ISET), 
     1       NETSIDE(1, ISET), SAMPRATE(ISET), BITS(1,ISET), 
     2       TAPEMODE(ISET), S2MODE, VALID, NS2USD, IS2USD )
      ELSE
         WRITE( MSGTXT, '( A, A, A )' )
     1       'VXS2MD2: S2 Modes with DAR = ', THEDAR,
     2       ' wiring not implemented, using naive S2 mode '
         IF( DOWARN )
     1       CALL WLOG( 1, MSGTXT ) 
         IF( DEBUG ) CALL WLOG( 1,'       Using VXS2DF2 ')
         CALL VXS2DF2( NCHAN(ISET), SAMPRATE(ISET), BITS(1,ISET), 
