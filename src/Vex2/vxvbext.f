      SUBROUTINE VXVBEXT( KS, ICH, EXTSYNTH1, EXTSYNTH2,
     1                   EXTSYNTH3, VBCOM, EXTFILTER )
C
...
C
      ...
      INTEGER        SYNPRT
      LOGICAL        GOTIT
      CHARACTER      LOPRT*20, FETXT*6, FILTTXT*6
      CHARACTER      EXTSYNTH1*(*), EXTSYNTH2*(*), EXTFILTER*(*)
      CHARACTER      VBCOM*(*)
      INTEGER        EXTSYNTH3
C ------------------------------------------------------------
      IF( DEBUG ) CALL WLOG( 0, 'VXVBRX2 starting.' )
C
      GOTIT = .FALSE.
C
...
C
          ...
          IF( SETSTA(1,KS)(1:4) .EQ. 'VLBA' ) THEN
             WRITE( EXTSYNTH1, '( I6 )' ) NINT( SYNTH(1,KS) * 1.D3 )
             WRITE( EXTSYNTH2, '( I6 )' ) NINT( SYNTH(2,KS) * 1.D3 )
...
C
      ...
      IF( GOTIT ) THEN
         EXTSYNTH3 = SYNPRT
         WRITE( VBCOM, '( A )' ) FETXT(1:LEN1(FETXT))
         WRITE( EXTFILTER, '( A )' ) FILTTXT(1:LEN1(FILTTXT))
C         WRITE( VBCOM, '( 1X, A, 2X, A, I6, 1X, A )' )
C     1       LOPRT(1:LEN1(LOPRT)), FETXT(1:LEN1(FETXT)), SYNPRT,
C     2       FILTTXT(1:LEN1(FILTTXT))
      ELSE
         EXTSYNTH3 = 0
         VBCOM = ' '
         EXTFILTER = ' '
