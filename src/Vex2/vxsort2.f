      SUBROUTINE VXSORT2( BLOCK, NXXVEX, XXISSET, NMODXX, IMODXX,
...
C     
      ...
      LOGICAL NEWFND, VXCFFQ2, VXCFIF2, VXCFBB2, VXCFTR2, VXCFHP2
      LOGICAL VXCFRL2, VXCFPO2, VXCFPH2
      LOGICAL PROBLEM
C ----------------------------------------------------------------------
C
...
C
      WRITE( MSGTXT, '( A, A, A )' ) 
     1    ' VXSORT2: Start sorting ', BLOCK, ' definition'
...
C
                     IF( BLOCK .EQ. 'FQ' ) THEN
                        IF( VXCFFQ2( KS, JS ) ) THEN
                           NEWFND = .FALSE.
                           SETISXX(KS) = IXX
                        END IF
                     ELSE IF( BLOCK .EQ. 'IF' ) THEN
                        IF( VXCFIF2( KS, JS ) ) THEN
                           NEWFND = .FALSE.
                           SETISXX(KS) = IXX
                        END IF
                     ELSE IF( BLOCK .EQ. 'BB' ) THEN
                        IF( VXCFBB2( KS, JS ) ) THEN
                           NEWFND = .FALSE.
                           SETISXX(KS) = IXX
                        END IF
                     ELSE IF( BLOCK .EQ. 'TR' ) THEN
                        IF( VXCFTR2( KS, JS ) ) THEN
                           NEWFND = .FALSE.
                           SETISXX(KS) = IXX
                        END IF
                     ELSE IF( BLOCK .EQ. 'HP' ) THEN
                        IF( VXCFHP2( KS, JS ) ) THEN
                           NEWFND = .FALSE.
                           SETISXX(KS) = IXX
                        END IF
                     ELSE IF( BLOCK .EQ. 'PH' ) THEN
                        IF( VXCFPH2( KS, JS ) ) THEN
                           NEWFND = .FALSE.
                           SETISXX(KS) = IXX
                        END IF
                     ELSE IF( BLOCK .EQ. 'PO' ) THEN
                        IF( VXCFPO2( KS, JS ) ) THEN
                           NEWFND = .FALSE.
                           SETISXX(KS) = IXX
                        END IF
                     ELSE IF( BLOCK .EQ. 'RL' ) THEN
                        IF( VXCFRL2( KS, JS ) ) THEN
                           NEWFND = .FALSE.
                           SETISXX(KS) = IXX
                        END IF
                     ELSE
                        CALL ERRLOG(' VXSORT2: unsupported BLOCK def ')
                     END IF
                  END IF
               END DO
            END IF
...
C
     ...
     1             'VXSORT2: Number of $'//BLOCK//' defs '//
...
C
               CALL VXNMXX2( BLOCK, NXXVEX, XXLINK )
C
            END IF
         END IF
      END DO
