...
C From commented code:     INTEGER JSCN, RECSCN, ISET, VXGTST, J
      ...
      LOGICAL SKIPPED
C ----------------------------------------------------------------------
      IF( DEBUG ) CALL WLOG( 1, 'VXSCNS2: Starting.' )
C
...
C           
            IF( IMODE .LT. 0 ) THEN
               CALL WLOG(1, 'VXSCNS2: Unexpected Mode'//
     1              ' encountered ')
               MSGTXT = ' '
     ...
     2             SETFILE(SETNUM(ISCN))(1:NC), ' skipped?'
               CALL ERRLOG( 'VXSCNS2: Fix setups' )
            END IF
C		 
...
C
                 ...
                 IF (IFS.GT.MFSET) THEN
                    CALL ERRLOG('VXSCNS2: Too many frequency'//
     1                  ' mode changes ')
                    STOP
                 END IF
C
...
C
     ...
     .                     'VXSCNS2: Telescope: ', STANAME(ISTA),
...
C
              ...
              IF( DEBUG ) THEN
                 MSGTXT = ' '
                 WRITE( MSGTXT, '( A, I3 )' )
     .               'VXSCNS2: Unused or new mode in scan ',ISCN
                 CALL WLOG( 1, MSGTXT )
              END IF
C
