...
C
     ...
     1        RLLINK(IRL)(1:LEN1(RLLINK(IRL))), SEP
         CALL VXSTLI2( IRL, NSTARL, ISTARL )
C
...
C
      IF( DOWARN ) 
     1       CALL WLOG( 1,'VXWRRL2: WARNING, barrel rolling not yet '//
     2       'permitted for the JIVE correlator')
      IF( DEBUG ) CALL WLOG( 1, 'VXWRRL2: Done with setups to VEX.' )
C
      RETURN
      END
