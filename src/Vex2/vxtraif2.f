      SUBROUTINE VXTRAIF2(ISCN, NMODE, OMODE, INTERVAL)
C
...
C
      ...
      INCLUDE 'vxlink2.inc'
C
      LOGICAL VXCFIF2, FOUND
      REAL INTERVAL
      INTEGER NMODE, OMODE, ISCN, JSCN
C
      CHARACTER VXNMIF2*32
C
...
C
     ...
     1          'VXTRAIF2: Number of $IF defs exceeding MAXMOD,'//
     2          '  array size...')
C
...
C
            IFLINK(NIFVEX) = VXNMIF2( NIFVEX, .TRUE. )
            CALL VXUNQL2( NIFVEX, IFLINK )
         END IF
C     
