...
C
     ...
     1             3X, A1, 1X, A1, 1X, A1, F9.2, 1X, A, 1X, 
     2             A1, 1X, A1)' ) 
     3             'if_def = ', LNK, 'IF_', 
...
C
C              Fill in receiver_name line:
C
               CALL VXVBRX2( KS, ICH, VBCOM )               
               I1 = LEN1( VBCOM )
               MSGTXT = ' '
C
               WRITE( MSGTXT, '( 5X, A, A1, A, A, 1X, A1, 1X, 
     1             A, 1X, A1, 1X)' ) 
     2             'receiver_name = ', LNK, 'IF_',
     3             IFCHAN(ICH,KS)(1:LEN1(IFCHAN(ICH,KS))), COL,
     4             VBCOM(1:I1), SEP
               I1 = LEN1( MSGTXT )
               MSGTXT = MSGTXT(1:I1)
               WRITE( IVEX, '( A )' ) MSGTXT(1:LEN1(MSGTXT))
