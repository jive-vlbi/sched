...
C -------------------------------------------------------------
      IF (DEBUG) CALL WLOG( 1,'VXSCH2: Start VEX SCHED section ')
      LINE = ' '      
      WARNFS = .FALSE.
      WARNGP = .FALSE.
      WARNBANK = .FALSE.
...
C
  ...
  200             CONTINUE
                  ...
                  END IF
               END IF
C               IF( ANNOT(ISCN) .NE. ' ' ) THEN
C                  WRITE( IVEX, '( A1, 5X, A )' ) COM, 
C     1               'Note a COMMENT was inserted during scheduling: '
C                  WRITE( IVEX, '( A1, 7X, A )' ) COM,
C     1                      ANNOT(ISCN)(1:MCH)
C                  INPAGE = INPAGE + 1
C                  IF( JCH1 .GT. 1 ) THEN
C                     WRITE( IVEX, '( A1, 10X, A )' ) COM, 
C     1                      ANNOT(ISCN)(JCH1:JCH2)
C                     INPAGE = INPAGE + 1
C                  END IF
C               END IF
            END IF
...
C
     ...
     1                  'IN2NET' ) THEN
              CALL PRTSCN( ISCN, 'VXSCH2' )
              WRITE ( MSGTXT, '(A, A, A)' ) 
     1          'VXSCH2: You have requested a GRABTO (ftp) scan, but ',
     ...
     3          'DATAPATH=IN2DISK'
            ...
            IF( GRABTO(ISCN) .EQ. 'NET' ) THEN
              CALL PRTSCN( ISCN, 'VXSCH2' )
              WRITE ( MSGTXT, '(A, A)' ) 
     1          'VXSCH2: You have requested GRABTO=NET, but ',
...
C              check the transfer time is consistent with the scan length
     ...
     1                  TRANSTAR .GT. TRANEND) THEN
                 CALL WRTMSG( 0, 'VXSCH2', 'vexgrabtime')
                 CALL PRTSCN( ISCN, 'VXSCH2' )
                 WRITE( MSGTXT, '( A, A, A, A )' )
     1              'VXSCH2:   WARNING: You have scheduled a GRABTO ',
...
C                      
     ...
     1              'VXSCH2: You have scheduled an ftp or eVLBI ',
...
C                 Previously restricted to VLBA, applies to all from 2024-08-14 (Comarc)
C
...
C
     ...
     2         'VXSCH2: ', STATION(STANUM(ISTA)), ' has ',
     3         NTSYS(ISTA), 
...
C
     ...
     1         'VXSCH2: ', STATION(STANUM(ISTA)), ': only ',
     ...
     3         ' Tsys measurements are on-source'
      ...
      END DO
      IF( TSYSMESS ) THEN
         CALL WLOG( 1, 'VXSCH2: Stations listed above are affected ' //
     1        'by slewing during Tsys calibration')
         CALL WRTMSG( 0, 'VXSCH2', 'tsysoffsrc' )
      END IF
C
      IF( WARNFS ) 
     1    CALL WLOG( 1,'VXSCH2: WARNING: Scan timing problem '//
     2    'for PCFS, this VEX will NOT run!!!!')
      IF( WARNGP ) THEN
         CALL WRTMSG( 0, 'VXSCH2', 'ftpgap' )
      END IF
C
      IF( WARNBANK ) THEN
C
