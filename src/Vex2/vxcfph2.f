      LOGICAL FUNCTION VXCFPH2( ISET, JSET )
C
...
C
         DO ICH = 1, NCHAN(ISET)
            ...
            ELSE IF( SIDE1(ICH,ISET) .EQ. 'L' ) THEN
               ISIDE1 = -1
            ELSE
               CALL ERRLOG('VXCFPH2: First LO sideband problem.' )
            END IF
...
C
         CALL VXTON2_2(-1, ISET, NCHAN(ISET), LOISUM,
...
C
         CALL VXTON2_2(-1, JSET, NCHAN(JSET), LOJSUM,
