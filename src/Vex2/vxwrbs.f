...
C     MARKIII was no longer needed.
      WRITE( IVEX, '( A, A1 )' ) '$BITSTREAMS', SEP     
C
...
C
            ...
            WRITE( IVEX, '( A )' ) COMLIN
         ELSE
            CALL ERRLOG( 'VSWRTR:  No VEX BS/DS blocks requested. '//
     1       'Something wrong.' )
         END IF
C
...
C         CALL VXSTLI2( ITR, NSTATR, ISTATR )
C
...
C     2         '    firmware_type = ', DBE(KS)(1:LEN1(DBE(KS))), SEP
C         IF( FIRMFILE(KS) .NE. ' ' )
C     1       WRITE( IVEX, '( A1, A, A, A )' ) COM,
C     2         '    firmware_file = ', 
...
C                
     ...
     1               COM, 'stream_sample_rate = ',
     2               SAMPRATE(KS)/FANOUT(KS),
     3               ' Ms/sec; tape speed low dens:', NINT(SPEEDL(KS)),
     4               'ips, high dens:', NINT(SPEEDH(KS)), 'ips'
              ELSE IF( USEDISK(ISCAT) ) THEN
                 WRITE( IVEX, '( 5X, A, F6.2, A )' ) 
     1               'stream_sample_rate =', SAMPRATE(KS)/FANOUT(KS), 
     2               ' Ms/sec;'
              END IF
C          
...
C          
     ...
     1                  COM, 'stream_sample_rate = ', SAMPRATE(KS), 
     2            ' Mb/s/IF;'
C          
C             Then find S2 mode
C                
...
C                
     ...
     1                     TRACK(ICH,IP,KS) .GT. 33 ) THEN
                       ...
                       ENDIF
                       WRITE( LINE(1:39), '( 5X, A, 1X, 
     1                     A1, A, I2.2, 1X, A1, 1X, A, 1X, A1, 
     2                     1X, I2, 1X )' ) 
     3                     'stream_def =', LNK, 'CH', ICH, 
     4                     COL, THEBIT, COL, BSTR
                       BSTR = BSTR + 1
...
C          
     ...
     1                           1X, A1, 1X, I2, 1X )' ) COL, ODBSTRN
                             ODBSTRN = ODBSTRN + 1
C     2                           TRACK1+(IFAN-1)*2+
C     3                           (IBIT-1)*2*NINT(FANOUT(KS))
                          END DO                          
                       ELSE IF( FORMAT(KS)(1:4) .EQ. 'VDIF' ) THEN
C
...
C     1                       ITRK = ITRK + 2
                          WRITE( LINE(LPOS:LPOS+4), '(1X, A1, 1X, I2)') 
     1                       COL, ODBSTRN
                          ODBSTRN = ODBSTRN + 1
                       END IF
