...
C
      ...
      IF( EXISTS .AND. ( OVERWRIT ) ) THEN
         OPSTAT = 'OLD'
      ELSE IF( EXISTS ) THEN
         CALL WLOG( 1, 'VXWRT2: '//VEXFILE//' already exists.' )
         CALL ERRLOG( 'VXWRT2: You need to delete old output files' )
      ELSE
         OPSTAT = 'NEW'
      END IF
C     
C     OPEN VEX file 
C
      CALL WLOG( 1, 'VXWRT2 :  Writing  V E X 2 file ' //
...
C      
      IF( NSTA .GT. 40 ) THEN
         WRITE( MSGTXT, '( A, I3, A )' ) 
     1       'VXWRT2: WARNING: More than 40 stations (', NSTA,
     2       ') in this schedule.' 
         CALL WLOG( 1,MSGTXT)
         VIOLFS = .TRUE.
      END IF
...
C
      IF( SCANL-SCAN1 .GT. 20000 ) THEN
         MSGTXT = ' '
         WRITE( MSGTXT, '( A, I6, A )' ) 
     1       'VXWRT2: WARNING: More than 20000 scans (', (SCANL-SCAN1),
     2       ') in this schedule.' 
         CALL WLOG( 1,MSGTXT)
         IF( ALLVLBA ) THEN
         ...
         DO ISTA = 1, NSTA-1
            DO JSTA = ISTA+1, NSTA
               IF( STCODE(STANUM(ISTA)) .EQ. STCODE(STANUM(JSTA)) ) THEN
                  WRITE( MSGTXT, '( A, A2, A )' )
     1                'VXWRT2: WARNING: The station code ', 
...
C     
      IF( VIOLFS )
     1    CALL WLOG( 1,'VXWRT2: WARNING Violating the limits '//
     2       'of PCFS, this VEX will NOT run!!!!')
C
C     
C     Initialize MODSET, NMODFQ, and NMODIF arrays to remove entries 
C     of vex file.
C
      DO I=1,MAXSTA
          DO J=1,MAXMOD
...
C
      CLOSE( UNIT=IVEX )
C
      IF( DEBUG ) CALL WLOG( 1, 'VXWRT2: Done with VEX2 section' )
      RETURN
      END
