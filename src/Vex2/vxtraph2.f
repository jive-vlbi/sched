      SUBROUTINE VXTRAPH2(ISCN, NMODE, OMODE, INTERVAL, NTONE,
     1    NTONTMP, ITONTMP )
C
...
C
      LOGICAL VXCFPH2, FOUND
      CHARACTER VXNMPH2*32
C
...
C
     ...
     1       'VXTRAPH2: Number of $PH defs exceeding MAXMOD '//
     2       'array size...')
C
...
C
               IF( VXCFPH2(NSETUP(INSCAN,ISTA), PHISSET(NPHVEX))) THEN 
...
C
         PHLINK(NPHVEX) = VXNMPH2( NPHVEX, .TRUE. )
         CALL VXUNQL2( NPHVEX, PHLINK )
C     
