      SUBROUTINE VXS2VL2( NDATCH, BBC, BBCSD, MBITSC, BSAMPL, NGROUP, 
...
C
      ...
      IF ( OKRATE ) THEN
         WRITE( MSGTXT, '( A )' )
     1       'VXS2VL2: Unable to match input channels config.'
         CALL WLOG( 1, MSGTXT )
      ELSE
         WRITE( MSGTXT, '( A, I2, A, I2, A, F5.2, A )' ) 
     1       'VXS2VL2: S2 cannot record', NDATCH, 'chan ',BSAMPL,
...
C
  100 VALID = .TRUE.
     ...
     1       ' VXS2VL2: Inconsistent Tapemode for this S2 Mode' )
      END IF
      S2MODE = S2M(IMODE)
