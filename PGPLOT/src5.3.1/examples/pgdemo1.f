...
C sinc (sin x over x) function and its Hilbert transform.
C-----------------------------------------------------------------------
      INTEGER NPT
      PARAMETER (NPT=200)
      INTEGER I
      REAL XR(NPT), YR1(NPT), YR2(NPT)
      REAL X, XMIN, XMAX
C
      CALL PGSAVE
      CALL PGSLW(1)
      XMIN = -5.0
      XMAX = 5.0
      CALL PGENV(XMIN, XMAX, -0.75, 1.2, 0, 1)
      CALL PGLAB('x/\gp', 'y(x)', 
     $     'PGPLOT Example 2:  Sinc Functions')
      DO 20 I=1,NPT
          XR(I) = XMIN + REAL(I-1)*(XMAX-XMIN)/REAL(NPT-1)
          X  = XR(I)*3.141592653
          YR2(I) = 0.0
          IF (X.NE.0.0) THEN
             YR1(I) = SIN(X)/X
   ...
   20 CONTINUE
      CALL PGSLW(2)
      CALL PGSCI(3)
      CALL PGLINE(NPT,XR,YR1)
      CALL PGTEXT(2.0, 1.0, '\fiy\fr = sin\fix\fr/\fix')
      CALL PGSCI(2)
      CALL PGLINE(NPT,XR,YR2)
      CALL PGTEXT(2.0, 0.85, '\fiy\fr = (cos\fix\fr - 1)/\fix')
      CALL PGUNSA
C-----------------------------------------------------------------------
      END
...
C
   ...
   20 CONTINUE
C
C Set background and foreground colors, and fill view surface
C with background.
C
      CALL PGBBUF
      CALL PGSAVE
      ...
      CALL PGERAS
C
C Select a square viewport/window.
C
      CALL PGSCH(0.5)
      CALL PGVSTD
      CALL PGWNAD(-1.05, 1.05, -1.05, 1.05)
C
C Box and title.
C
      CALL PGSCI(2)
...
C-----------------------------------------------------------------------
      ...
      SUBROUTINE PGEX16
C
C Test routines PGSHD1 and PGSHD2. 
C-----------------------------------------------------------------------
      REAL PI
      PARAMETER (PI=3.141596253)
      INTEGER N
      PARAMETER (N=300)
      REAL X(N), Y1(N), Y2(N)
      INTEGER I
...
C
 ...
 20   CONTINUE
      ...
      CALL PGSCH(1.5)
      CALL PGVSTD
      CALL PGSWIN(0.0, 2.0*PI, -1.1, 1.1)
      CALL PGSCI(7)
      CALL PGSFS(1)
      CALL PGSHD2(N, X, Y1, Y2)
      CALL PGSLW(3)
      CALL PGSCI(2)
      CALL PGLINE(N, X, Y1)
      CALL PGSCI(3)
      ...
      CALL PGLAB('x', 'y', 'Shading with PGSHD2: y = sin(x), cos(2x)')
      CALL PGEBUF
      CALL PGUNSA
C
C Test PGSHD1.
C
 ...
 10   CONTINUE
      ...
      CALL PGSCH(1.5)
      CALL PGVSTD
      CALL PGSWIN(0.0, 2.0*PI, 0.0, 2.1)
      CALL PGSCI(5)
      CALL PGSFS(3)
      CALL PGSHD1(N, X, Y1, 0.0)
      CALL PGSLW(3)
      CALL PGSCI(2)
      CALL PGLINE(N, X, Y1)
      CALL PGSLW(1)
      ...
      CALL PGBOX('BCNST', 0.0, 0, 'BCNSTV', 0.0, 0)
      CALL PGLAB('x', 'y',
     :     'Shading with PGSHD1: y = |sin(x) - cos(2x)|')
      CALL PGEBUF
      CALL PGUNSA
C-----------------------------------------------------------------------
      END
      ...
      SUBROUTINE PGEX17
C-----------------------------------------------------------------------
C This example illustrates the use of PGARC and PGPIE.
C-----------------------------------------------------------------------
      INTEGER I, J
      REAL X(4), Y(4)
      DATA X/5.0, 15.0, 5.0, 15.0/
      DATA Y/15.0, 15.0, 5.0, 5.0/
C
C Call PGENV to specify the range of the axes and to draw a box, and
C PGLAB to label it. 
C
      CALL PGENV(0.,20.,0.,20.,1,0)
      CALL PGLAB(' ', ' ', 'PGPLOT Example: PGPIE and PGARC')

      CALL PGSLW(2)
      DO 20 J=1,4
         CALL PGSFS(J)
C
C Draw pie segments and arcs.
C
         CALL PGSCI(3)
         CALL PGSHS(45.0, 1.0, 0.0)
         DO 10 I=0,3
            CALL PGPIE(X(J), Y(J), 4.0, 45.0 + I*90.0, 90.0 + I*90.0)
            CALL PGARC(X(J), Y(J), 4.5, 45.0 + I*90.0, 90.0 + I*90.0)
 10      CONTINUE
         CALL PGSCI(2)
         CALL PGSHS(45.0, 1.0, 0.333)
         CALL PGPIE(X(J), Y(J), 3.0, 0.0, 90.0)
         CALL PGPIE(X(J), Y(J), 3.0, 180.0, 270.0)
C
C Draw a circle.
C
         CALL PGSCI(1)
         CALL PGSHS(45.0, 1.0, 0.667)
         CALL PGCIRC(X(J), Y(J), 2.0)
 20   CONTINUE
C-----------------------------------------------------------------------
      END
