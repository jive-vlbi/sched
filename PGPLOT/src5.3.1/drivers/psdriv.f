...
C Version 6.8  - 2002 Oct 24 - get %%Title from PS_TITLE.
C Version 6.9  - 2003 Jun 30 - remove erroneous number left on PS stack.
C Version 7.0  - 2005 Jan  6 - add /EPS mode.
C
C Supported device: 
...
C   /EPS (encapsulated PostScript, colour, portrait)
C
C Default file name:
C   pgplot.ps
C
...
C    6.0 inches horizontal by 4.5 inches vertical (EPS).
...
C-----------------------------------------------------------------------
      ...
      CHARACTER*(*) PTYPE, LTYPE, CPTYPE, CLTYPE, EPTYPE, DEFNAM
      PARAMETER (
     ...
     : EPTYPE='EPS   (Encapsulated Postscript, colour)')
...
C     -- Color mode?
      ...
      COLOR = L.GT.0 .OR. MODE.EQ.3 .OR. MODE.EQ.4 .OR. MODE.EQ.5
      IF (COLOR) THEN
         DO 91 CI=0,15
            RVALUE(CI) = RINIT(CI)
            GVALUE(CI) = GINIT(CI)
...
C        -- machine-dependent!
 ...
 94         IF (CHR(LCHR:LCHR).EQ.' ') THEN
      ...
      CALL GRGENV('PS_TITLE', INSTR, L)
      IF (L.GT.0) CALL GRPS02(IOERR, UNIT, '%%Title: '//INSTR(1:L))
      CALL GRPS02(IOERR, UNIT, '%%Creator: PGPLOT [PSDRIV 7.0]')
      CALL GRDATE(INSTR, L)
  ...
  100 CONTINUE
     ...
     1       'writing PostScript file: file is incomplete')
      ...
      IF (.NOT.STDOUT) THEN
         ...
         CALL GRFLUN(UNIT)
      END IF
      IF (MODE.EQ.5 .AND. NPAGE.GT.1) THEN
         CALL GRWARN('Warning: EPS file contains more than one page: '//
     :               FNAME(:LFNAME))
      END IF
      STATE = 0
      RETURN
...
C
  110 CONTINUE
     ...
     1               OFFH, 0, 0)
      ...
      END IF
      CALL GRPS02(IOERR, UNIT, INSTR(:L))
      CALL GRPS02(IOERR, UNIT, '1 setlinejoin 1 setlinecap 1 LW')
...
C
      IF (IER.EQ.0) THEN
          ...
          IF (IER.NE.0) CALL 
     1        GRWARN('Error writing PostScript file')
      END IF
C-----------------------------------------------------------------------
      END
