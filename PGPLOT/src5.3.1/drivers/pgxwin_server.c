...
static int xw_get_colorcells ARGS((XWServer *xw, PGwin *pgw, XVisualInfo *vi,
				   Colormap cmap, unsigned long mincol,
 ...
 */
  pgw->color.pixel=(unsigned long *)malloc(sizeof(unsigned long) *
					   XW_MAX_COLORS);
 ...
 * class and at least 'mincol' colors. The pgw->color structure will
 ...
 *                               members of pgw->color will be set as
 *                               indicated.
 *                                vi    -  The info descriptor of the
 *                                         located visual.
 *                                cmap  -  The ID of the located colormap.
 *                                pixel -  The color cell pixel indexes.
 *                                ncol  -  The number of colors available.
 *                               Note that the returned colormap and
 *                               pixels must be deleted via xw_del_Colormap()
 *                               and the vi structure must be deleted using
 *                               XFree() when the window is destroyed.
 */
#ifdef __STDC__
...
#endif
{
  ...
  unsigned long mincol; /* The minimum acceptable number of colors. */
  unsigned long maxcol; /* The maximum number of colors to allocate. */
/*
 * Get the mininum acceptable number of colors.
 */
  mincol = pgw->mincol;
/*
 * When allocating unshared colors, we should heed the user's
 * requested maximum, since colors are a limited resource partitioned
 * between different applications. When allocating shared colors,
 * there is no reason to allocate any fewer than the maximum allowable
 * by pgplot, since we aren't competing with other applications for
 * colors.
 */
  switch(class) {
  case PseudoColor:        /* Un-shared (read/write) colors */
  case GrayScale:
    maxcol = pgw->maxcol;
    break;
  case StaticColor:        /* Shared (readonly) colors */
  case DirectColor:
  case TrueColor:
  case StaticGray:
  default:
    maxcol = XW_MAX_COLORS;
    break;
  };
/*
 ...
 */
  if(class == pgw->color.vi->class) {
    ...
    ncol = xw_get_colorcells(xw, pgw, pgw->color.vi, cmap, mincol, maxcol);
    if(ncol >= maxcol) {
      pgw->color.cmap = cmap;
      pgw->color.ncol = ncol;
 ...
 * best matches 'maxcol'. Note that the colormap_size memeber of
...
/*
 * If available, use a visual that has at least 'maxcol' independant
 * colors.
 */
      if(vi_above)
	vi = vi_above;
...
/*
 * Allocate color-cells in the new colormap. Since this is a private colormap,
 * so there is no contention for colors between the calling program and
 * other applications, attempt to get the maximum number of colors that
 * PGPLOT can handle, regardless of any maximum stipulated by the user.
 */
    } else if((ncol = xw_get_colorcells(xw, pgw, vi, cmap, mincol,
					XW_MAX_COLORS)) >= mincol) {
      XFree((char *) pgw->color.vi);
      pgw->color.vi   = vi;
      pgw->color.cmap = cmap;
      pgw->color.ncol = ncol;
 ...
 *  xw           XWServer *  The PGPLOT /xw server descriptor.
 *  pgw             PGwin *  The PGPLOT window descriptor.
 *  vi        XVisualInfo *  The information descriptor of the visual
 *                           containing the colormap.
 *  cmap         Colormap    The colormap ID to associate the cells with.
 *  mincol  unsigned long    The minimum acceptable number of colors.
 *  maxcol  unsigned long    The maximum number of colors to allocate.
 * Output:
...
static int xw_get_colorcells(XWServer *xw, PGwin *pgw, XVisualInfo *vi,
			     ...
			     unsigned long maxcol)
#else
static int xw_get_colorcells(xw, pgw, vi, cmap, mincol, maxcol)
     XWServer *xw; PGwin *pgw; XVisualInfo *vi; Colormap cmap;
     unsigned long mincol; unsigned long maxcol;
#endif
{
  unsigned long cmapsize; /* The number of colors in the colormap */
 ...
 */
	...
	if(rgb_mask & (unsigned long)0x1)
	  cmapsize <<= (unsigned long)1;
      } while(cmapsize < maxcol && (rgb_mask >>= (unsigned long)1) != 0);
    };
    break;
  default:
    cmapsize = 0;
...
/*
 * If the color-map contains less colors than the user-requested upper
 * limit, reduce the upper limit to match the number that are potentially
 * available.
 */
  if(cmapsize < maxcol)
    maxcol = cmapsize;
 ...
 */
	...
	ncol = maxcol;
/*
 * If there aren't at least mincol color cells available, then
 * give up on this colormap.
 */
      } else if(!XAllocColorCells(xw->display, cmap, False, planes, nplanes,
				  pgw->color.pixel, (unsigned) mincol)) {
	ncol = 0;
      } else {
/*
 * Since we were able to allocate mincol cells, we may be able to
 * allocate more. First discard the mincol cells, so that we can
 * try for a bigger number.
 */
	XFreeColors(xw->display, cmap, pgw->color.pixel, (int) mincol,
		    (unsigned long)0);
/*
