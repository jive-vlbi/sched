...
C 10-Feb-1999 - null character terminates string [TJP].
C 25-Mar-1999 - add thin space character [TJP].
C-----------------------------------------------------------------------
      CHARACTER*8  FONTS
      CHARACTER*48 GREEK
...
C
      ...
      J = 0
C
C Get next character; treat non-printing characters as spaces; null
C character terminates string.
C
  100 J = J+1
...
C
     ...
     1                     TEXT(J+1:J+1).EQ.'B') THEN
            ...
            ELSE IF (TEXT(J+1:J+1).EQ.'.') THEN
                ...
                GOTO 100
            ELSE IF (TEXT(J+1:J+1).EQ.',') THEN
C               -- thin space (8 units, half normal space)
                NSYMBS = NSYMBS + 1
                SYMBOL(NSYMBS) = 2198
                J = J+1
                GOTO 100
