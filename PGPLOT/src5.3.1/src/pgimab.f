C*PGIMAB -- color image from a 2D data array, with blanking
C%void cpgimab(const float *a, int idim, int jdim, int i1, int i2, \
C% int j1, int j2, float a1, float a2, const float *tr, float bval, int bi);
C+
      SUBROUTINE PGIMAB (A, IDIM, JDIM, I1, I2, J1, J2,
     1                   A1, A2, TR, BVAL, BI)
      INTEGER IDIM, JDIM, I1, I2, J1, J2, BI
      REAL    A(IDIM,JDIM), A1, A2, TR(6), BVAL
C
...
C
C Pixels with value equal to BVAL are "blanked" and are assigned the
C color index BI (e.g., use 0 for background color).
C
...
C                    world coordinates.
C  BVAL   (input)  : magic value for blanked pixels.
C  BI     (input)  : color index to use for blanked pixels.
C--
C 29-Jan-2004: new routine, adapted from PGIMAG [TJP].
C-----------------------------------------------------------------------
      INCLUDE  'pgplot.inc'
      REAL PA(6)
      LOGICAL PGNOTO
...
C Check inputs.
C
      IF (PGNOTO('PGIMAB')) RETURN
      IF (I1.LT.1 .OR. I2.GT.IDIM .OR. I1.GT.I2 .OR.
     1    J1.LT.1 .OR. J2.GT.JDIM .OR. J1.GT.J2) THEN
          CALL GRWARN('PGIMAB: invalid range I1:I2, J1:J2')
      ELSE IF (A1.EQ.A2) THEN
          CALL GRWARN('PGIMAB: foreground level = background level')
      ELSE IF (PGMNCI(PGID).GT.PGMXCI(PGID)) THEN
          CALL GRWARN('PGIMAB: not enough colors available')
      ELSE
C
...
C
          ...
          CALL GRIMB0(A, IDIM, JDIM, I1, I2, J1, J2, A1, A2, PA,
     :                PGMNCI(PGID), PGMXCI(PGID), PGITF(PGID), BVAL, BI)
          CALL PGEBUF
      END IF
C-----------------------------------------------------------------------
      END
